# Reproduce Mind of Thought on Spatial-2
...
- Verified evaluation manifest: `/home/tione/notebook/tianjungu/Mind_of_Thought/data/eval_samples.json`
...
```bash
cd /home/tione/notebook/tianjungu/Mind_of_Thought
python3 -m py_compile runtime_config.py   scripts/mind_of_thought.py   scripts/mind_of_thought_pipeline.py   scripts/mind_of_thought_baseline.py   scripts/results_parser.py   core/semantic_labeler.py
...
`/home/tione/notebook/tianjungu/Mind_of_Thought/outputs/mind_of_thought_<timestamp>`
...
```bash
cd /home/tione/notebook/tianjungu/Mind_of_Thought
python3 scripts/results_parser.py --base /path/to/run_dir_with_gpu_subdirs
```

## 6. Reproduce the shard-to-shard comparison against the original internal reference

Original verified shard reference:

- The private internal `gpu2` reference shard used during the migration validation on `Spatial-2`

Equivalent Mind of Thought shard command:

```bash
cd /home/tione/notebook/tianjungu/Mind_of_Thought
python3 scripts/mind_of_thought.py   --full   --gpu_id 2   --num_gpus 8   --device cuda:2   --output-dir outputs/compare_gpu2_manual
```
...
On `2026-03-26`, the live comparison against the original internal `gpu2` shard matched the first scene (`42899696`) exactly on the first 17 emitted question results, including `question_type`, `prediction`, `ground_truth`, and `score`.
