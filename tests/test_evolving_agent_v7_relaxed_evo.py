#!/usr/bin/env python3
"""
Self-Evolving Agent V7 Relaxed Evolution - 宽松演化版本

核心设计：
1. 所有任务同时保存两种推理结果
2. **宽松的演化策略**：降低触发阈值，确保演化带来正向收益
3. 演化思路：
   - counting: 检测重复计数，使用中位数修正 (原有)
   - size: 多帧尺寸取中位数，减少单帧噪声
   - direction: 增强多帧投票，使用 bbox 重叠分析
   - distance: 多帧深度一致性投票
   - appearance: 置信度加权确认首帧
...
class MindMapEvolver:
            ...
            if max_count > median_count * 2 and max_count > 2:
                ...
                if new_count != target_entity.count:
                    actions.append(EvolutionAction(
                        ...
                        new_value=new_count,
                        reasoning=f"Max count ({max_count}) >> median ({median_count:.1f})",
                        confidence=0.7,
                    ))
    ...
    def evolve_for_size(
        ...
        mind_map: Dict[str, MindMapEntity],
        target_object: str,
    ) -> Tuple[Dict[str, MindMapEntity], List[EvolutionAction], float]:
        """针对 size 任务 - 返回多帧中位数尺寸估计（不修改心智地图，只返回推荐值）"""
        actions = []
        ...
        if target_entity is None:
            return mind_map, actions, None
        
        # 收集所有帧的尺寸估计
        sizes = []
        for det in target_entity.detections:
            size = max(det.estimated_height, det.estimated_width)
            if size > 0:
                sizes.append(size)
        ...
        if len(sizes) >= 2:
            ...
            original_size = target_entity.get_estimated_size()
            
            # 只要有多帧数据，就用中位数
            if abs(original_size - median_size) / max(median_size, 0.01) > 0.1:
                recommended_size = median_size
                actions.append(EvolutionAction(
                    action_type='size_recommendation',
                    target_entity=target_label,
                    old_value=original_size,
                    new_value=median_size,
                    reasoning=f"Multi-frame median: {len(sizes)} frames, median={median_size:.3f}m vs original={original_size:.3f}m",
                    confidence=min(0.9, 0.5 + len(sizes) * 0.05),
                ))
            else:
                recommended_size = original_size
        
        return mind_map, actions, recommended_size
    ...
    def evolve_for_direction(
        ...
        obj1_name: str,
        obj2_name: str,
    ) -> Tuple[Dict[str, MindMapEntity], List[EvolutionAction], Dict]:
        """针对 direction 任务 - 返回多帧投票的方向结果"""
        actions = []
        direction_votes = {'left': 0, 'right': 0, 'above': 0, 'below': 0, 'front': 0, 'behind': 0}
        
        ent1 = ent2 = None
        for label, entity in mind_map.items():
            if match_object_name(obj1_name, label):
                ent1 = entity
            if match_object_name(obj2_name, label):
                ent2 = entity
        
        if not ent1 or not ent2:
            return mind_map, actions, direction_votes
        
        frame_dets1 = {d.frame_idx: d for d in ent1.detections}
        frame_dets2 = {d.frame_idx: d for d in ent2.detections}
        common_frames = set(frame_dets1.keys()) & set(frame_dets2.keys())
        
        for frame_idx in common_frames:
            d1, d2 = frame_dets1[frame_idx], frame_dets2[frame_idx]
            ...
            # 水平方向（图像左右）
            if cx1 - cx2 > 30:
                direction_votes['right'] += 1
            elif cx2 - cx1 > 30:
                direction_votes['left'] += 1
            
            # 垂直方向（图像上下）
            if cy1 - cy2 > 30:
                direction_votes['below'] += 1
            elif cy2 - cy1 > 30:
                direction_votes['above'] += 1
            
            # 深度方向
            if d1.depth - d2.depth > 0.3:
                direction_votes['behind'] += 1
            elif d2.depth - d1.depth > 0.3:
                direction_votes['front'] += 1
        
        if common_frames:
            actions.append(EvolutionAction(
                action_type='direction_vote',
                target_entity=f"{obj1_name}_vs_{obj2_name}",
                old_value=None,
                new_value=direction_votes,
                reasoning=f"Multi-frame vote from {len(common_frames)} frames",
                confidence=len(common_frames) / max(len(ent1.detections), len(ent2.detections)),
            ))
    ...
    def evolve_for_distance(
        ...
        obj1_name: str,
        obj2_name: str,
    ) -> Tuple[Dict[str, MindMapEntity], List[EvolutionAction], str]:
        """针对 rel_distance 任务 - 返回多帧投票的距离比较结果"""
        actions = []
        closer_vote = {obj1_name: 0, obj2_name: 0, 'equal': 0}
        
        ref_ent = obj1_ent = obj2_ent = None
        for label, entity in mind_map.items():
            if match_object_name(ref_obj, label):
                ref_ent = entity
            if match_object_name(obj1_name, label):
                obj1_ent = entity
            if match_object_name(obj2_name, label):
        ...
        if not ref_ent or not obj1_ent or not obj2_ent:
            return mind_map, actions, None
        
        ref_dets = {d.frame_idx: d for d in ref_ent.detections}
        obj1_dets = {d.frame_idx: d for d in obj1_ent.detections}
        obj2_dets = {d.frame_idx: d for d in obj2_ent.detections}
        
        common_frames = set(ref_dets.keys()) & set(obj1_dets.keys()) & set(obj2_dets.keys())
        
        for frame_idx in common_frames:
            ref_d = ref_dets[frame_idx]
            obj1_d = obj1_dets[frame_idx]
            obj2_d = obj2_dets[frame_idx]
            
            # 计算到参考物体的距离（使用 3D 位置或深度）
            dist1 = abs(obj1_d.depth - ref_d.depth)
            dist2 = abs(obj2_d.depth - ref_d.depth)
            
            # 也考虑 2D 距离
            ref_cx = (ref_d.bbox[0] + ref_d.bbox[2]) / 2
            ref_cy = (ref_d.bbox[1] + ref_d.bbox[3]) / 2
            obj1_cx = (obj1_d.bbox[0] + obj1_d.bbox[2]) / 2
            obj1_cy = (obj1_d.bbox[1] + obj1_d.bbox[3]) / 2
            obj2_cx = (obj2_d.bbox[0] + obj2_d.bbox[2]) / 2
            obj2_cy = (obj2_d.bbox[1] + obj2_d.bbox[3]) / 2
            
            dist1_2d = np.sqrt((obj1_cx - ref_cx)**2 + (obj1_cy - ref_cy)**2)
            dist2_2d = np.sqrt((obj2_cx - ref_cx)**2 + (obj2_cy - ref_cy)**2)
            
            # 综合深度和 2D 距离
            score1 = dist1 + dist1_2d * 0.01  # 2D 距离权重较小
            score2 = dist2 + dist2_2d * 0.01
            
            if score1 < score2 * 0.9:
                closer_vote[obj1_name] += 1
            elif score2 < score1 * 0.9:
                closer_vote[obj2_name] += 1
            else:
                closer_vote['equal'] += 1
        ...
        winner = None
        if common_frames:
            if closer_vote[obj1_name] > closer_vote[obj2_name]:
                winner = obj1_name
            elif closer_vote[obj2_name] > closer_vote[obj1_name]:
                winner = obj2_name
            
            actions.append(EvolutionAction(
                action_type='distance_vote',
                target_entity=f"{obj1_name}_vs_{obj2_name}_to_{ref_obj}",
                old_value=None,
                new_value=closer_vote,
                reasoning=f"Multi-frame vote from {len(common_frames)} frames",
                confidence=len(common_frames) / 10,
            ))
        
        return mind_map, actions, winner
    
    def evolve_for_appearance(
        self,
        mind_map: Dict[str, MindMapEntity],
        target_objects: List[str],
    ) -> Tuple[Dict[str, MindMapEntity], List[EvolutionAction], Dict]:
        """针对 appearance 任务 - 返回置信度加权的首次出现帧"""
        actions = []
        object_first_frame = {}
        ...
        for target in target_objects:
            for label, entity in mind_map.items():
                if match_object_name(target, label):
                    # 收集所有检测，按帧排序
                    sorted_dets = sorted(entity.detections, key=lambda x: x.frame_idx)
                    ...
                    for det in sorted_dets:
                        if det.confidence >= 0.3:
                            object_first_frame[target] = det.frame_idx
                            
                            # 如果与原始 first_seen_frame 不同，记录演化
                            if entity.first_seen_frame != det.frame_idx:
                                actions.append(EvolutionAction(
                                    action_type='confirm_first_frame',
                                    target_entity=label,
                                    old_value=entity.first_seen_frame,
                                    new_value=det.frame_idx,
                                    reasoning=f"First confident detection (conf={det.confidence:.2f}) at frame {det.frame_idx}",
                                    confidence=det.confidence,
                                ))
                                entity.first_seen_frame = det.frame_idx
                            break
                    else:
                        # 没有高置信度检测，使用最早的
                        object_first_frame[target] = sorted_dets[0].frame_idx
                    break
        
        return mind_map, actions, object_first_frame
        ...
        try:
            ...
            if question_type == 'object_counting' and target_objects:
                ...
                if match:
                    ...
                    mind_map, actions, recommended_size = evolver.evolve_for_size(mind_map, target_obj)
                    evolution_actions.extend(actions)
                    if recommended_size is not None:
                        evo_result = {'size': recommended_size}
                ...
                for pattern in patterns:
                    ...
                    if match:
                        obj1, obj2 = match.groups()
                        mind_map, actions, direction_votes = evolver.evolve_for_direction(mind_map, obj1, obj2)
                        evolution_actions.extend(actions)
                        evo_result = {'direction_votes': direction_votes, 'obj1': obj1, 'obj2': obj2}
                        break
            
            elif question_type == 'object_rel_distance':
                # 提取: Which is closer to X, Y or Z?
                match = re.search(r'closer to (?:the )?(\w+)[,\s]+(?:the )?(\w+) or (?:the )?(\w+)', question.lower())
                if match:
                    ref_obj, obj1, obj2 = match.groups()
                    mind_map, actions, winner = evolver.evolve_for_distance(mind_map, ref_obj, obj1, obj2)
                    evolution_actions.extend(actions)
                    evo_result = {'closer': winner, 'ref': ref_obj, 'obj1': obj1, 'obj2': obj2}
            
            elif question_type == 'obj_appearance_order':
                opt_content = re.sub(r'^[A-D]\.\s*', '', options[0]) if options else ""
                appearance_objects = [obj.strip().lower() for obj in opt_content.split(',')]
                mind_map, actions, first_frames = evolver.evolve_for_appearance(mind_map, appearance_objects)
                evolution_actions.extend(actions)
                evo_result = {'first_frames': first_frames}
            ...
            if question_type == 'object_counting':
                ...
                # 如果有演化推荐的尺寸，优先使用
                if evo_result and 'size' in evo_result:
                    size_cm = evo_result['size'] * calibration.scale_factor * 100
                    rule_pred = str(int(size_cm))
                    rule_reasoning = f"Evolution recommended size: {evo_result['size']:.3f}m (calibrated: {size_cm:.0f}cm)"
                else:
                ...
                # 如果有演化投票结果，尝试用它来验证/修正
                if evo_result and 'direction_votes' in evo_result:
                    votes = evo_result['direction_votes']
                    max_vote_dir = max(votes, key=votes.get)
                    if votes[max_vote_dir] >= 3:  # 至少 3 帧投票
                        # 尝试匹配选项
                        for i, opt in enumerate(options):
                            if max_vote_dir in opt.lower():
                                evo_pred = chr(65 + i)
                                if evo_pred != rule_pred:
                                    rule_reasoning += f" | Evolution vote: {max_vote_dir}({votes[max_vote_dir]} frames) -> {evo_pred}"
                                    # 如果演化投票明显（>=5帧），覆盖规则结果
                                    if votes[max_vote_dir] >= 5:
                                        rule_pred = evo_pred
                                break
                ...
                # 使用演化的距离投票
                if evo_result and 'closer' in evo_result and evo_result['closer']:
                    closer_obj = evo_result['closer']
                    for i, opt in enumerate(options):
                        if closer_obj in opt.lower():
                            evo_pred = chr(65 + i)
                            if evo_pred != rule_pred:
                                rule_reasoning += f" | Evolution closer: {closer_obj} -> {evo_pred}"
                                rule_pred = evo_pred  # 距离判断，演化结果更可靠
                            break
...
def main():
    ...
    # 打印结果
    print("\n" + "=" * 100)
    print("V7 Relaxed Evolution (宽松演化版本) - 测试结果")
    print("=" * 100)
    ...
    output_dir = Path(args.output_dir) / f"evolving_agent_v7_relaxed_{timestamp}"
