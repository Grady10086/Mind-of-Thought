...
核心改进:
1. 废弃 Gram-Schmidt - 它假设 camera_right 方向大致正确
2. 使用叉积从 viewer_forward 和 camera_up 直接计算 viewer_right
3. viewer_right = viewer_forward × camera_up (右手定则)

如果 DA3 的 camera_up 不可靠，尝试多种策略:
- 策略1: 使用 camera_up
- 策略2: 使用标准 Y 轴 [0, 1, 0] 作为近似上方向
- 策略3: 使用纯 2D 叉积 (在 XZ 平面上)
"""
...
# 修复同义词问题 - 移除 dishwasher <-> washer 的映射
SYNONYM_MAP = {
    'sofa': ['couch', 'settee'],
...
def compute_direction_cross_product(
    target_world: np.ndarray,
    standing_world: np.ndarray,
    facing_world: np.ndarray,
    camera_up: np.ndarray,
    strategy: str = 'camera_up',
) -> Tuple[str, Dict]:
    """
    使用叉积计算方向
    
    关键公式:
    viewer_forward = facing - standing (观察者面朝方向)
    viewer_right = viewer_forward × up_vector (右手定则)
    
    然后用点积判断 target 在哪个方向:
    - proj_forward = target_rel · viewer_forward (前后)
    - proj_right = target_rel · viewer_right (左右)
    
    策略:
    - 'camera_up': 使用 DA3 相机的 up 向量
    - 'y_axis': 使用标准 Y 轴 [0, 1, 0]
    - 'neg_y_axis': 使用负 Y 轴 [0, -1, 0] (如果 DA3 的 Y 向下)
    """
    ...
    # 选择 up 向量
    if strategy == 'camera_up':
        up_vector = camera_up
    elif strategy == 'y_axis':
        up_vector = np.array([0, 1, 0])
    elif strategy == 'neg_y_axis':
        up_vector = np.array([0, -1, 0])
    elif strategy == 'z_axis':
        up_vector = np.array([0, 0, 1])
    else:
        up_vector = camera_up
    
    # 使用叉积计算 viewer_right
    # viewer_right = viewer_forward × up_vector
    viewer_right = np.cross(viewer_forward, up_vector)
    viewer_right_norm = np.linalg.norm(viewer_right)
    
    if viewer_right_norm < 1e-6:
        # viewer_forward 与 up_vector 平行，尝试备选
        if strategy != 'y_axis':
            viewer_right = np.cross(viewer_forward, np.array([0, 1, 0]))
            viewer_right_norm = np.linalg.norm(viewer_right)
        if viewer_right_norm < 1e-6:
            return "unknown", {'error': 'viewer_forward parallel to up'}
...
def worker_process(gpu_id: int, samples: List[Dict], output_file: str, strategy: str = 'camera_up'):
    ...
    for sample in tqdm(samples, desc=f"GPU {gpu_id} [{strategy}]"):
        ...
        try:
            ...
            if (target_entity and target_entity.get('position_3d') is not None and
                ...
                if np.allclose(standing_pos, facing_pos, atol=1e-3):
                    ...
                    debug_info = {'error': 'standing and facing have same position'}
                else:
                    predicted_direction, debug_info = compute_direction_cross_product(
...
def print_stats(results: List[Dict], strategy: str):
...
def main():
    ...
    parser.add_argument('--strategy', type=str, default='all', 
                        choices=['camera_up', 'y_axis', 'neg_y_axis', 'z_axis', 'all'])
    ...
    output_dir = Path(args.output_dir) / f"v710_cross_product_{timestamp}"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    strategies = ['camera_up', 'y_axis', 'neg_y_axis', 'z_axis'] if args.strategy == 'all' else [args.strategy]
    
    best_strategy = None
    best_acc = 0
    
    for strategy in strategies:
        output_file = str(output_dir / f"results_{strategy}.json")
        results = worker_process(args.gpu, samples, output_file, strategy)
        acc, found_acc = print_stats(results, strategy)
    ...
    print(f"\n最佳策略: {best_strategy} (找到物体时准确率: {best_acc:.1f}%)")
    print(f"\n结果保存到: {output_dir}")
