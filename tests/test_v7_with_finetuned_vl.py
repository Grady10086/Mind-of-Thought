...
def worker_process(gpu_id: int, samples: List[Dict], vl_model_path: str, adapter_path: str, output_dir: str):
    """GPU Worker 进程 - 使用文件系统传递结果避免Queue死锁"""
    # 在子进程中添加DA3路径
    import sys
    da3_paths = [
        '/home/tione/notebook/tianjungu/projects/Depth-Anything-3',
        '/home/tione/notebook/tianjungu/projects/Depth-Anything-3/src'
    ]
    ...
    # 直接使用指定的GPU，不依赖CUDA_VISIBLE_DEVICES
    device = f'cuda:{gpu_id}'
    torch.cuda.set_device(gpu_id)
    
    logger.info(f"GPU {gpu_id}: Worker process started, using device: {device}, current device: {torch.cuda.current_device()}")
    
    # 创建组件（使用指定的GPU设备）
    builder = MindMapBuilder(device=device, num_frames=32, box_threshold=0.25)
    calibrator = ScaleCalibrator()
    evolver = MindMapEvolver(device=device)
    qa = DualReasoningQA(device=device)
    
    # 显式加载深度估计和语义标注模型
    try:
        builder.load_models()
        logger.info(f"GPU {gpu_id}: Depth and labeling models loaded successfully")
    except Exception as e:
        logger.error(f"GPU {gpu_id}: Failed to load depth/labeling models: {e}")
        import traceback
        traceback.print_exc()
        # 保存空结果到文件
        import json
    ...
    # 加载 VL 模型 (with adapter if provided)
    vl_loaded = False
    try:
        qa.load_vl_model(vl_model_path, adapter_path)
        vl_loaded = True
    ...
    import json
    result_file = os.path.join(output_dir, f'results_gpu{gpu_id}.json')
    logger.info(f"GPU {gpu_id}: Saving {len(results)} results to {result_file}")
    with open(result_file, 'w') as f:
        json.dump(results, f, indent=2)
    logger.info(f"GPU {gpu_id}: Worker process completed")
...
def main():
    # 注意：不设置spawn方式，使用默认的fork方式（在Linux上更可靠）
    ...
    for gpu_id in range(num_gpus):
        p = mp.Process(
            ...
            args=(gpu_id, gpu_samples[gpu_id], args.vl_model, args.adapter_path, args.output_dir)
        )
        p.start()
        processes.append(p)
    ...
    all_results = []
    for gpu_id in range(num_gpus):
        result_file = os.path.join(args.output_dir, f'results_gpu{gpu_id}.json')
        if os.path.exists(result_file):
            with open(result_file, 'r') as f:
                results = json.load(f)
                all_results.extend(results)
                logger.info(f"Loaded {len(results)} results from GPU {gpu_id}")
        else:
            logger.warning(f"Result file not found for GPU {gpu_id}: {result_file}")
