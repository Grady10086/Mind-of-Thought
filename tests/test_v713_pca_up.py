#!/usr/bin/env python3
"""
V7.13 - 使用 PCA 估计场景地面平面

核心思想:
室内场景中的物体大多分布在地面附近。使用 PCA 分析所有物体的 3D 位置，
最小方差方向应该近似垂直于地面。

算法:
1. 收集场景中所有检测到的物体的 3D 中心点
2. 对这些点进行 PCA 分析
3. 最小特征值对应的特征向量应该近似垂直于地面（即"上"方向）
4. 使用这个估计的"上"方向进行叉积计算
"""
...
def estimate_ground_normal_pca(points_3d: np.ndarray) -> np.ndarray:
    """
    使用 PCA 估计地面法向量
...
def compute_direction_with_pca_up(
    ...
    if viewer_right_norm < 1e-6:
        ...
        viewer_right_norm = np.linalg.norm(viewer_right)
        if viewer_right_norm < 1e-6:
            viewer_right = np.cross(viewer_forward, np.array([0, 0, 1]))
            viewer_right_norm = np.linalg.norm(viewer_right)
            if viewer_right_norm < 1e-6:
                return "unknown", {'error': 'cannot determine right'}
    ...
    for sample in tqdm(samples, desc=f"GPU {gpu_id}"):
        ...
        try:
            ...
            for label, entity in mind_map_3d.items():
                if entity.get('position_3d') is not None:
                    all_positions.append(entity['position_3d'])
            
            # 使用 PCA 估计地面法向量
            if len(all_positions) >= 3:
                points_3d = np.array(all_positions)
                up_vector = estimate_ground_normal_pca(points_3d)
            else:
            ...
            if (target_entity and target_entity.get('position_3d') is not None and
                ...
                if np.allclose(standing_pos, facing_pos, atol=1e-3):
                    ...
                    predicted_direction, debug_info = compute_direction_with_pca_up(
                        target_pos, standing_pos, facing_pos, up_vector,
                    )
...
def main():
    ...
    output_dir = Path(args.output_dir) / f"v713_pca_up_{timestamp}"
