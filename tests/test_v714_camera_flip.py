...
核心问题分析:
在 V7.10 neg_y_axis 中，有些案例 proj_forward 的符号是错的。
这可能是因为:
1. DA3 坐标系的 Y 轴方向不一致
2. 叉积的顺序影响了 viewer_right 的方向

尝试策略:
- 基于 proj_forward 的符号一致性检测
- 如果 viewer_forward 的方向明显不对，尝试翻转
"""
...
def compute_direction_robust(
    target_world: np.ndarray,
    standing_world: np.ndarray,
    facing_world: np.ndarray,
    camera_forward: np.ndarray,  # 相机的前方向
) -> Tuple[str, Dict]:
    """
    更鲁棒的方向计算
    
    关键改进:
    1. 使用相机的 forward 方向来确定 "up" 的符号
    2. 如果 viewer_forward 与 camera_forward 的夹角 > 90 度，翻转叉积结果
    """
    ...
    if viewer_right_norm < 1e-6:
        ...
        viewer_right_norm = np.linalg.norm(viewer_right)
        if viewer_right_norm < 1e-6:
            return "unknown", {'error': 'cannot compute viewer_right'}
    ...
    # 关键：检查是否需要翻转左右方向
    # 如果相机是从"后面"看观察者的，那么左右会反过来
    # 使用相机 forward 与观察者 forward 的夹角来判断
    camera_observer_cos = np.dot(camera_forward, viewer_forward)
    flip_lr = camera_observer_cos < 0  # 如果相机从后面看，翻转左右
    ...
    debug_info = {
        ...
        'camera_forward': camera_forward.tolist(),
        'camera_observer_cos': float(camera_observer_cos),
        'flip_lr': flip_lr,
    }
...
def extract_camera_basis(extrinsics: np.ndarray):
    R = extrinsics[:3, :3]
    ...
    for sample in tqdm(samples, desc=f"GPU {gpu_id}"):
        ...
        try:
            ...
            if da3_prediction is not None and da3_prediction.extrinsics is not None:
                _, _, camera_forward = extract_camera_basis(da3_prediction.extrinsics[0])
            else:
            ...
            if (target_entity and target_entity.get('position_3d') is not None and
                ...
                if np.allclose(standing_pos, facing_pos, atol=1e-3):
                    ...
                    predicted_direction, debug_info = compute_direction_robust(
                        target_pos, standing_pos, facing_pos, camera_forward,
                    )
...
def main():
    ...
    output_dir = Path(args.output_dir) / f"v714_camera_flip_{timestamp}"
