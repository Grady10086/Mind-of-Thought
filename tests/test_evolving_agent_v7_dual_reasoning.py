#!/usr/bin/env python3
"""
Self-Evolving Agent V7 - 双推理模式框架 (规则 + Qwen3-VL)

核心设计：
1. 所有任务同时保存两种推理结果：
   - rule_prediction: 基于心智地图的规则推理结果
   - vl_prediction: 基于 Qwen3-VL-8B-Instruct 的视觉语言推理结果
2. 使用 Qwen3-VL-8B-Instruct 作为 VL 模型
...
# ============================================================================
# 演化器 - 任务特定Evolution (覆盖所有任务类型)
# ============================================================================

class MindMapEvolver:
    """任务特定的心智地图演化器 - 覆盖所有任务类型"""
    ...
    def evolve(
        ...
        question: str,
        question_type: str,
        sampled_frames: List[np.ndarray] = None,
        ...
        if 'counting' in question_type:
            mind_map, actions = self._evolve_counting(mind_map, question)
        elif 'size' in question_type:
            mind_map, actions = self._evolve_size(mind_map, question)
        elif 'abs_distance' in question_type:
            mind_map, actions = self._evolve_distance(mind_map, question)
        elif 'direction' in question_type:
            mind_map, actions = self._evolve_direction(mind_map, question)
        elif 'appearance' in question_type or 'order' in question_type:
            mind_map, actions = self._evolve_appearance_order(mind_map, question)
        elif 'rel_distance' in question_type:
            mind_map, actions = self._evolve_direction(mind_map, question)
        elif 'route' in question_type:
            mind_map, actions = self._evolve_route(mind_map, question)
        else:
            mind_map, actions = self._evolve_size(mind_map, question)
    ...
    # --- Counting ---
    def _evolve_counting(
        self,
        mind_map: Dict[str, MindMapEntity],
        question: str,
    ) -> Tuple[Dict[str, MindMapEntity], List[EvolutionAction]]:
        """Counting: 去重 + 中位数统计 + 置信度过滤"""
        actions = []
        ...
        for label, entity in mind_map.items():
            ...
            if len(counts) >= 3:
                ...
                evolved_count = median_count if median_count == mode_count else min(median_count, mode_count)
                confidence = 0.9 if median_count == mode_count else 0.7
            else:
            ...
            if high_conf_dets:
                hc_frame_counts = defaultdict(int)
                for det in high_conf_dets:
                    hc_frame_counts[det.frame_idx] += 1
                if hc_frame_counts:
                    hc_median = int(np.median(list(hc_frame_counts.values())))
                    if hc_median < evolved_count:
                        evolved_count = hc_median
            ...
            if evolved_count != original_count:
                ...
                actions.append(EvolutionAction(
                    ...
                    reasoning=f"Multi-frame: median={np.median(counts):.1f}, mode={max(set(counts), key=counts.count)}, std={np.std(counts):.2f}",
    ...
    def _evolve_size(
        ...
        for label, entity in mind_map.items():
            if entity.size_3d is None:
                continue
            for cal_obj, std_size in self.calibration_refs.items():
                if cal_obj in label.lower():
                    est = max(entity.size_3d[0], entity.size_3d[1])
                    if est > 0.01:
                        factor = std_size / est
                        if 0.2 < factor < 5.0 and entity.avg_confidence > best_conf:
        ...
        if calibration_source:
            actions.append(EvolutionAction(
                action_type='size_calibration',
                target_entity='global_scale',
                old_value=1.0,
                new_value=calibration_factor,
                reasoning=f"Using {calibration_source} (conf={best_conf:.2f})",
        ...
        for label, entity in mind_map.items():
            ...
            # 物理约束
            for obj_type, (min_s, max_s) in self.typical_sizes.items():
                if obj_type in label.lower():
                    cur = max(entity.size_3d[0], entity.size_3d[1])
                    if cur < min_s * 0.3 or cur > max_s * 3:
                        target = (min_s + max_s) / 2
                        correction = target / cur if cur > 0 else 1
                        entity.size_3d = entity.size_3d * correction
                        actions.append(EvolutionAction(
                            action_type='physics_constraint',
                            target_entity=label,
                            old_value=f"{cur:.2f}m",
                            new_value=f"{max(entity.size_3d):.2f}m",
                            reasoning=f"Outside [{min_s:.1f}, {max_s:.1f}]m for {obj_type}",
                            confidence=0.7,
                        ))
                    break
    ...
    # --- Distance ---
    def _evolve_distance(
        self,
        mind_map: Dict[str, MindMapEntity],
        question: str,
    ) -> Tuple[Dict[str, MindMapEntity], List[EvolutionAction]]:
        """Distance: 继承Size校准 + 室内距离约束"""
        actions = []
        
        mind_map, size_actions = self._evolve_size(mind_map, question)
        ...
        if len(positions) >= 2:
            ...
            max_depth = np.max(np.abs(positions_arr[:, 2]))
            if max_depth > 10.0:
                scale = 10.0 / max_depth * 0.9
                for entity in mind_map.values():
                    if entity.position_3d is not None:
                ...
                actions.append(EvolutionAction(
                    ...
                    reasoning="Scaled to indoor constraint (<10m)",
                    confidence=0.75,
                ))
    ...
    # --- Direction ---
    def _evolve_direction(
        self,
        mind_map: Dict[str, MindMapEntity],
        question: str,
    ) -> Tuple[Dict[str, MindMapEntity], List[EvolutionAction]]:
        """Direction: 多帧位置聚合 + 位置稳定性分析"""
        actions = []
        
        # 先做基础校准
        mind_map, base_actions = self._evolve_size(mind_map, question)
        actions.extend(base_actions)
        
        # 多帧位置聚合
        for label, entity in mind_map.items():
            if not entity.detections or len(entity.detections) < 3:
                continue
            
            frame_positions = [d.position_3d for d in entity.detections if d.position_3d is not None]
            if len(frame_positions) < 3:
                continue
            
            positions_arr = np.array(frame_positions)
            median_pos = np.median(positions_arr, axis=0)
            original_pos = entity.position_3d.copy() if entity.position_3d is not None else None
            
            pos_std = np.std(positions_arr, axis=0)
            stability = 1.0 / (1.0 + np.mean(pos_std))
            
            entity.position_3d = median_pos
            
            if original_pos is not None:
                change = np.linalg.norm(median_pos - original_pos)
                if change > 0.1:
                    actions.append(EvolutionAction(
                        action_type='position_aggregation',
                        target_entity=label,
                        old_value=f"({original_pos[0]:.2f},{original_pos[1]:.2f},{original_pos[2]:.2f})",
                        new_value=f"({median_pos[0]:.2f},{median_pos[1]:.2f},{median_pos[2]:.2f})",
                        reasoning=f"Aggregated {len(frame_positions)} frames, stability={stability:.2f}",
    ...
    # --- Appearance Order ---
    def _evolve_appearance_order(
        self,
        mind_map: Dict[str, MindMapEntity],
        question: str,
    ) -> Tuple[Dict[str, MindMapEntity], List[EvolutionAction]]:
        """Appearance: 置信度加权首现帧 + 连续性分析"""
        actions = []
        ...
        for label, entity in mind_map.items():
            ...
            # 找第一个高置信度帧
            first_hc_frame = None
            for fidx in sorted(frame_conf.keys()):
                if max(frame_conf[fidx]) > 0.4:
                    first_hc_frame = fidx
                    break
            if first_hc_frame is None:
                first_hc_frame = min(frame_conf.keys())
            
            # 连续性
            frames = sorted(frame_conf.keys())
            if len(frames) >= 2:
                gaps = [frames[i+1] - frames[i] for i in range(len(frames)-1)]
                continuity = 1.0 if max(gaps) < np.mean(gaps) * 3 else 0.5
            else:
                continuity = 0.5
            
            original = entity.first_seen_frame
            if first_hc_frame != original:
                entity.first_seen_frame = first_hc_frame
                actions.append(EvolutionAction(
                    action_type='first_frame_correction',
                    target_entity=label,
                    old_value=original,
                    new_value=first_hc_frame,
                    reasoning=f"High-conf (>0.4) first frame, continuity={continuity:.2f}",
    ...
    def _evolve_route(
        ...
        # 功能区域识别
        area_keywords = {
            'kitchen': ['stove', 'refrigerator', 'fridge', 'sink', 'microwave', 'oven'],
            'bedroom': ['bed', 'nightstand', 'dresser', 'closet'],
            'living_room': ['sofa', 'couch', 'tv', 'television'],
        ...
        for label in mind_map.keys():
            ...
            for area, keywords in area_keywords.items():
                if any(k in ll for k in keywords):
                    detected_areas[area].append(label)
                    break
        ...
        if detected_areas:
            actions.append(EvolutionAction(
                ...
                old_value='unknown',
                new_value=str(dict(detected_areas)),
                reasoning=f"Detected {len(detected_areas)} functional areas",
                confidence=0.7,
            ))
        
        # 空间拓扑
        objects_pos = [(l, e.position_3d) for l, e in mind_map.items() if e.position_3d is not None]
        if len(objects_pos) >= 2:
            positions = np.array([p for _, p in objects_pos])
            dists = []
            ...
            actions.append(EvolutionAction(
                ...
                target_entity='scene',
                old_value='raw_positions',
                new_value=f"avg_dist={np.mean(dists):.2f}m, max_dist={np.max(dists):.2f}m, n={len(objects_pos)}",
                reasoning=f"Built spatial topology",
                confidence=0.8,
            ))
    ...
    # 保持旧接口兼容
    def evolve_for_counting(self, mind_map, target_object, frames, frame_indices):
        """兼容旧接口"""
        question = f"How many {target_object}"
        return self._evolve_counting(mind_map, question)
...
Answer:"""
    ...
    # ========================================================================
    # VL-Driven Evolution: 两阶段推理
    # Stage 1: VL审视感知结果 → Evolution报告
    # Stage 2: VL基于审视报告 + 视频 → 最终答案
    # ========================================================================
    
    def vl_evolution_review(
        self,
        mind_map_text: str,
        question: str,
        question_type: str,
        ...
        """Stage 1: VL审视感知结果，输出evolution报告"""
        if self.vl_model is None:
            return ""
        
        # 根据任务类型生成针对性的审视prompt
        if 'counting' in question_type:
            review_focus = """Focus your review on:
- Are the object counts consistent across frames? (same object may be detected multiple times)
- Are there objects that should be counted but were missed?
- Are there false positives (wrong detections)?
- Is the detection confidence reliable?"""
        
        elif 'size' in question_type or 'room_size' in question_type:
            review_focus = """Focus your review on:
- Are the estimated sizes physically reasonable? (e.g., a chair should be ~0.8m, a door ~2.0m)
- Is the depth estimation consistent? (objects at similar distances should have similar depths)
- Can you identify reference objects with known sizes for calibration?
- Does the overall scale make sense for an indoor scene?"""
        
        elif 'distance' in question_type:
            review_focus = """Focus your review on:
- Are the 3D positions of the two target objects reasonable?
- Is the depth estimation consistent between them?
- Are there reference objects that can help verify the distance scale?
- Does the estimated distance make sense for an indoor scene (typically 0.5-10m)?"""
        
        elif 'direction' in question_type:
            review_focus = """Focus your review on:
- Are the relative positions of the target objects correct?
- Is the coordinate system consistent with the camera viewpoint?
- Do the position estimates seem stable (consistent across frames)?
- Could camera movement affect the perceived spatial relationships?"""
        
        elif 'appearance' in question_type or 'order' in question_type:
            review_focus = """Focus your review on:
- Are the first_seen_frame values reliable? (low confidence early detections may be wrong)
- Could some objects have been present but undetected in earlier frames?
- Is there a clear temporal pattern in the detections?
- Does the detection continuity suggest reliable temporal ordering?"""
        
        elif 'route' in question_type:
            review_focus = """Focus your review on:
- Does the spatial layout of objects suggest a logical room arrangement?
- Can you identify functional areas (kitchen, bedroom, bathroom)?
- Are the relative positions between objects consistent with the video?
- What is the most efficient path through the detected objects?"""
        
        else:
            review_focus = """Focus your review on:
- Are the detection results consistent with what you see in the video?
- Are there any obvious errors in positions, sizes, or counts?
- What corrections would improve the accuracy of the perception data?"""
        
        review_prompt = f"""You are a spatial perception quality reviewer. Your task is to review the perception system's output and identify potential issues.
...
Please provide a brief review (2-4 sentences) identifying:
1. Any issues or inconsistencies you notice
2. What corrections or adjustments might be needed
3. What you can verify from the video that the perception system might have gotten wrong

Review:"""
        
        review_response, _ = self._call_vl_model(review_prompt, video_path)
        return review_response
    
    def vl_answer_with_evolution(
        self,
        question: str,
        video_path: str,
        mind_map: Dict[str, MindMapEntity],
        ...
        # Stage 1: VL审视
        evolution_review = self.vl_evolution_review(
            mind_map_text, question, question_type, video_path, options
        )
        ...
        final_prompt = f"""You are a spatial intelligence assistant analyzing a video of an indoor scene.

=== DETECTED OBJECTS FROM PERCEPTION SYSTEM ===
{mind_map_text}

=== PERCEPTION REVIEW (self-check) ===
{evolution_review}
...
=== INSTRUCTIONS ===
Based on the perception data, the review above, and your own observation of the video:
1. Consider the issues identified in the review.
2. Use what you see in the video to verify or override the perception data where needed.
3. Provide your best answer.

{answer_instruction}

Answer:"""
        
        response, raw_response = self._call_vl_model(final_prompt, video_path)
        
        full_reasoning = f"Evolution Review: {evolution_review}\n---\nFinal VL: {raw_response}"
        
        if is_numerical:
            match = re.search(r'[\d.]+', response)
            if match:
                return match.group(), full_reasoning
...
def worker_process(gpu_id: int, samples: List[Dict], vl_model_path: str, result_queue: mp.Queue, enable_vl_evolution: bool = False):
    """GPU Worker 进程
    
    Args:
        enable_vl_evolution: 是否启用VL-driven Evolution (两阶段推理)
    """
    ...
    for sample in tqdm(samples, desc=f"GPU {gpu_id}"):
        ...
        try:
            ...
            # 3. Evolution (根据模式选择)
            evolution_actions = []
            if not enable_vl_evolution:
                # 规则Evolution (原始模式)
                mind_map, actions = evolver.evolve(
                    mind_map, question, question_type, sampled_frames
                )
                evolution_actions.extend(actions)
            # VL-driven Evolution在步骤4中通过两阶段推理实现
            
            # 4. 双推理模式 - 同时获取规则结果和 VL 结果
            rule_pred, rule_reasoning = "", ""
            vl_pred, vl_reasoning = "", ""
            
            if enable_vl_evolution and vl_loaded:
                # VL-driven Evolution: 两阶段推理 (对所有任务统一处理)
                vl_pred, vl_reasoning = qa.vl_answer_with_evolution(
                    question, video_path, mind_map, question_type,
                    options, calibration, total_frames
                )
                # 规则推理仍然执行作为参考
...
def main():
    parser = argparse.ArgumentParser(description='Self-Evolving Agent V7 - Dual Reasoning Mode')
    ...
    parser.add_argument('--vl-evolution', action='store_true', default=False,
                       help='Enable VL-driven Evolution (two-stage reasoning)')
    ...
    for gpu_id in range(num_gpus):
        p = mp.Process(
            ...
            args=(gpu_id, gpu_samples[gpu_id], args.vl_model, result_queue, args.vl_evolution)
        )
        p.start()
        processes.append(p)
    ...
    if args.vl_evolution:
        print("*** VL-Driven Evolution ENABLED (两阶段推理) ***")
    print("=" * 100)
    print(f"VL Model: {args.vl_model}")
    print(f"VL Evolution: {'ON' if args.vl_evolution else 'OFF'}")
    print("-" * 100)
