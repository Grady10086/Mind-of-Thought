#!/usr/bin/env python3
"""
Self-Evolving Agent V7.2 - 使用 DA3 1.1 的双推理模式框架 (规则 + Qwen3-VL)

改进：
- 将 DA3-Large (单帧深度) 替换为 DA3 1.1 (多帧 3D 重建)
- 使用 DA3 1.1 的相机位姿估计获得统一的世界坐标系
- 保留所有校准、演化和 VL 推理机制
...
│                    (DA3 1.1 + Dual Reasoning Mode)                          │
│                                                                             │
│  ┌──────────┐   ┌───────────┐    ┌──────────┐    ┌─────────┐               │
│  │ DA3 1.1  │ ->│ Calibrator│ -> │ Evolver  │ -> │ MindMap │               │
│  │ 3D重建   │   │标定物识别  │    │心智地图修正│    │ 校准后   │               │
│  └──────────┘   └───────────┘    └──────────┘    └────┬────┘               │
│                                                       │                     │
│                                    ┌──────────────────┴──────────────────┐  │
│                                    ▼                                     ▼  │
...
class MindMapBuilder:
    """心智地图构建器 - 使用 DA3 1.1 多帧 3D 重建"""
    
    def __init__(self, device: str = 'cuda', num_frames: int = 16, box_threshold: float = 0.25,
                 da3_model_path: str = "/home/tione/notebook/tianjungu/hf_cache/DA3NESTED-GIANT-LARGE-1.1"):
        self.device = device
    ...
    def load_models(self):
        ...
        if self._da3_estimator is None:
            ...
            self._da3_estimator = DA3FullEstimator(
                model_name=self.da3_model_path,
                device=self.device,
                use_ray_pose=True,
            )
    ...
    def build_from_video(self, video_path: str, target_objects: List[str] = None) -> Tuple[Dict[str, MindMapEntity], int, List[np.ndarray]]:
        ...
        if len(sampled_frames) < 2:
            return {}, total_frames, sampled_frames
        
        # 使用 DA3 1.1 进行多帧 3D 重建
        da3_prediction = self._da3_estimator.estimate_multiview(sampled_frames, ref_view_strategy="first")
        ...
        for i, frame_rgb in enumerate(sampled_frames):
            ...
            for det in detections:
                ...
                # 使用 DA3 计算 3D 位置 (使用相机坐标系，与 V7 保持一致)
                pos_3d = self._da3_estimator.compute_object_center_3d(
                    da3_prediction, i, bbox_scaled, 
                    use_camera_coords=True  # 关键：使用相机坐标系
                )
                
                if pos_3d is None:
                    # 如果无法获取 3D 位置，使用默认值而不是跳过
                    pos_3d = np.array([0.0, 0.0, 1.0])
                
                # 计算深度 (使用 Z 坐标)
                depth = float(pos_3d[2]) if pos_3d[2] > 0 else 1.0
                
                # 使用 DA3 的 compute_object_size_3d 获取精确尺寸
                # 这会使用 DA3 预测的真实内参进行计算
                est_width, est_height = self._da3_estimator.compute_object_size_3d(
                    da3_prediction, i, bbox_scaled
                )
...
def worker_process(gpu_id: int, samples: List[Dict], vl_model_path: str, result_queue: mp.Queue):
    ...
    builder = MindMapBuilder(
        device='cuda', 
        num_frames=32,  # 32 帧以获取更多信息
        box_threshold=0.25,
        da3_model_path="/home/tione/notebook/tianjungu/hf_cache/DA3NESTED-GIANT-LARGE-1.1"
    )
...
def main():
    ...
    # 打印结果
    print("\n" + "=" * 100)
    print("V7.2 双推理模式框架 (DA3 1.1 32帧 + 规则 + Qwen3-VL) - 测试结果")
    print("=" * 100)
    ...
    output_dir = Path(args.output_dir) / f"evolving_agent_v72_da3_11_32f_{timestamp}"
