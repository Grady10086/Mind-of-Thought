#!/usr/bin/env python3
"""
Self-Evolving Agent V7 Full Evolution - 完整演化版本
...
3. **完整的演化机制**：针对所有任务类型实现演化策略
4. 演化方法：
   - evolve_for_counting: 计数任务 (原有)
   - evolve_for_size: 尺寸估计 (交叉验证多帧)
   - evolve_for_direction: 方向判断 (多帧一致性校验)
   - evolve_for_distance: 距离估计 (遮挡检测与校正)
   - evolve_for_appearance: 出现顺序 (多帧追踪确认)
...
class MindMapEvolver:
    ...
    def evolve_for_size(
        ...
        if target_entity is None or len(target_entity.detections) < 2:
            return mind_map, actions
        ...
        valid_sizes = [s for s in sizes if lower <= s <= upper]
        if not valid_sizes:
            valid_sizes = sizes
        ...
        # 如果原始估计与中位数差异较大，进行校正
        if abs(original_size - median_size) / median_size > 0.3:
            # 更新所有检测的尺寸
            scale = median_size / original_size if original_size > 0 else 1.0
            for det in target_entity.detections:
                det.estimated_height *= scale
                det.estimated_width *= scale
            ...
            actions.append(EvolutionAction(
                ...
                reasoning=f"Cross-frame validation: {len(sizes)} detections, median={median_size:.3f}m, original={original_size:.3f}m, outliers removed: {len(sizes) - len(valid_sizes)}",
                confidence=0.8 if len(valid_sizes) >= 3 else 0.6,
            ))
    ...
    def evolve_for_direction(
        ...
        """针对 direction 任务演化心智地图 - 多帧位置一致性校验"""
        actions = []
        ...
        for frame_idx in common_frames:
            ...
            relative_positions.append((cx1 - cx2, cy1 - cy2, d1.depth - d2.depth))
        
        if len(relative_positions) < 2:
            return mind_map, actions
        
        # 检查一致性：计算各帧的相对位置方差
        dx_list = [p[0] for p in relative_positions]
        dy_list = [p[1] for p in relative_positions]
        dz_list = [p[2] for p in relative_positions]
        
        # 使用符号一致性检查：如果大多数帧方向一致，认为可靠
        dx_signs = [1 if x > 0 else -1 for x in dx_list]
        dy_signs = [1 if y > 0 else -1 for y in dy_list]
        
        dx_consistency = abs(sum(dx_signs)) / len(dx_signs)
        dy_consistency = abs(sum(dy_signs)) / len(dy_signs)
        
        # 如果 X/Y 方向的一致性很高，使用多帧中位数更新 3D 位置
        if dx_consistency > 0.7 or dy_consistency > 0.7:
            # 计算中位数相对位置
            median_dx = np.median(dx_list)
            median_dy = np.median(dy_list)
            median_dz = np.median(dz_list)
            
            # 检查是否需要更新位置
            if ent1.position_3d is not None and ent2.position_3d is not None:
                current_diff = ent1.position_3d - ent2.position_3d
                
                # 如果当前位置差与多帧中位数不一致（符号不同），进行校正
                if (current_diff[0] * median_dx < 0 and abs(median_dx) > 50):  # X 方向不一致
                    # 校正 ent1 的位置
                    old_pos = ent1.position_3d.copy()
                    ent1.position_3d[0] = ent2.position_3d[0] + (0.5 if median_dx > 0 else -0.5)
                    
                    actions.append(EvolutionAction(
                        action_type='correct_position_x',
                        target_entity=label1,
                        old_value=old_pos[0],
                        new_value=ent1.position_3d[0],
                        reasoning=f"Multi-frame direction consistency: {len(common_frames)} frames, X consistency={dx_consistency:.2f}",
                        confidence=dx_consistency,
                    ))
    ...
    def evolve_for_distance(
        ...
        """针对 rel_distance 任务演化心智地图 - 检测遮挡和深度估计问题"""
        actions = []
        ...
        for frame_idx in common_frames:
            d1, d2 = frame_dets1[frame_idx], frame_dets2[frame_idx]
            # 记录深度差：正值表示 obj1 更远
            depth_comparisons.append(d1.depth - d2.depth)
        
        if len(depth_comparisons) < 2:
            return mind_map, actions
        
        # 检查深度估计的一致性
        signs = [1 if d > 0.1 else (-1 if d < -0.1 else 0) for d in depth_comparisons]
        ...
        # 如果一致性高，使用多帧中位数校正
        if consistency > 0.6:
            median_depth_diff = np.median(depth_comparisons)
            
            if ent1.position_3d is not None and ent2.position_3d is not None:
                current_diff = ent1.position_3d[2] - ent2.position_3d[2]
                
                # 如果当前深度差与多帧估计不一致
                if (current_diff * median_depth_diff < 0) or abs(current_diff - median_depth_diff) > 1.0:
                    old_depth = ent1.position_3d[2]
                    ent1.position_3d[2] = ent2.position_3d[2] + median_depth_diff
                    
                    actions.append(EvolutionAction(
                        action_type='correct_depth',
                        target_entity=label1,
                        old_value=old_depth,
                        new_value=ent1.position_3d[2],
                        reasoning=f"Multi-frame depth consistency: {len(common_frames)} frames, consistency={consistency:.2f}, median_diff={median_depth_diff:.2f}m",
    ...
    def evolve_for_appearance_order(
        ...
        """针对 appearance_order 任务演化心智地图 - 多帧追踪确认首次出现帧"""
        actions = []
        ...
        for target in target_objects:
            ...
            # 检查首次出现帧的可靠性
            frame_indices = target_entity.get_frame_indices()
            if not frame_indices:
                continue
            
            # 获取前几帧的检测置信度
            early_frames = sorted(frame_indices)[:3]
            early_confidences = []
            for det in target_entity.detections:
                if det.frame_idx in early_frames:
                    early_confidences.append(det.confidence)
            
            if not early_confidences:
                continue
            
            avg_early_conf = np.mean(early_confidences)
            
            # 如果早期检测置信度太低，可能是误检
            if avg_early_conf < 0.3 and len(frame_indices) > 3:
                # 找到第一个高置信度检测帧
                sorted_dets = sorted(target_entity.detections, key=lambda x: x.frame_idx)
                for det in sorted_dets:
                    if det.confidence >= 0.4:
                        new_first_frame = det.frame_idx
                        old_first_frame = target_entity.first_seen_frame
                        
                        if new_first_frame != old_first_frame:
                            target_entity.first_seen_frame = new_first_frame
                            
                            actions.append(EvolutionAction(
                                action_type='correct_first_frame',
                                target_entity=target_label,
                                old_value=old_first_frame,
                                new_value=new_first_frame,
                                reasoning=f"Early detections low confidence ({avg_early_conf:.2f}), using first high-confidence frame",
                                confidence=0.7,
                            ))
                        break
        ...
        try:
            ...
            if question_type == 'object_counting' and target_objects:
                ...
                if match:
                    ...
                    mind_map, actions = evolver.evolve_for_size(mind_map, target_obj, calibration)
                    evolution_actions.extend(actions)
            
            elif 'direction' in question_type:
                # direction 演化：多帧位置一致性校验
                patterns = [
                    r'direction.*?(\w+).*?from.*?(\w+)',
                    r'(\w+).*?relative to.*?(\w+)',
                    r'where is (?:the )?(\w+).*?relative to (?:the )?(\w+)',
                ...
                for pattern in patterns:
                    ...
                    if match:
                        obj1, obj2 = match.groups()
                        mind_map, actions = evolver.evolve_for_direction(mind_map, obj1, obj2)
                        evolution_actions.extend(actions)
                        break
            
            elif question_type == 'object_rel_distance':
                # rel_distance 演化：深度估计一致性校验
                match = re.search(r'(\w+).*?closer.*?(\w+).*?or.*?(\w+)', question.lower())
                if match:
                    ref_obj = match.group(1)
                    obj1 = match.group(2)
                    obj2 = match.group(3)
                    # 分别对两对物体进行演化
                    mind_map, actions1 = evolver.evolve_for_distance(mind_map, obj1, ref_obj)
                    mind_map, actions2 = evolver.evolve_for_distance(mind_map, obj2, ref_obj)
                    evolution_actions.extend(actions1)
                    evolution_actions.extend(actions2)
            
            elif question_type == 'obj_appearance_order':
                # appearance 演化：确认首次出现帧
                opt_content = re.sub(r'^[A-D]\.\s*', '', options[0]) if options else ""
                appearance_objects = [obj.strip().lower() for obj in opt_content.split(',')]
                mind_map, actions = evolver.evolve_for_appearance_order(mind_map, appearance_objects)
                evolution_actions.extend(actions)
...
def main():
    ...
    # 打印结果
    print("\n" + "=" * 100)
    print("V7 Full Evolution (完整演化版本) - 测试结果")
    print("=" * 100)
    ...
    output_dir = Path(args.output_dir) / f"evolving_agent_v7_full_evo_{timestamp}"
