#!/usr/bin/env python3
"""
Self-Evolving Agent V7.2 - Counting 演化 + 空间锚点增强

核心改进（基于 V7 Counting 版本）：
1. 保持 V7 的 Counting 演化机制不变
2. 在 VL prompt 中添加语义空间锚点描述
3. 空间锚点：将 3D 位置转换为自然语言描述（如 "Chair_1 在 Table 后方 2 米"）
4. 仅增强方向/距离任务的 prompt，不改变 VL 推理流程

关键设计：
- 不使用 CoST（Chain-of-Spatial-Thought）替代原有 prompt
- 空间锚点作为 mind_map_text 的补充信息
- 保守增强，验证空间锚点是否能改善方向/距离任务

架构：
┌─────────────────────────────────────────────────────────────────────────────┐
│                      Self-Evolving Agent V7.2                               │
│                  (Counting Evolution + Spatial Anchor)                      │
│                                                                             │
│  ┌─────────┐    ┌───────────┐    ┌──────────┐    ┌─────────┐               │
│  │ 感知    │ -> │ Calibrator│ -> │ Evolver  │ -> │ MindMap │               │
│  │DA3+DINO │    │标定物识别  │    │Counting演化│   │ 校准后   │               │
│  └─────────┘    └───────────┘    └──────────┘    └────┬────┘               │
│                                                       │                     │
│                                    ┌──────────────────┴──────────────────┐  │
│                                    ▼                                     ▼  │
...
空间锚点示例：
- "Chair_1 is 1.5m behind the Table (reference object)"
- "Sofa is 2.3m to the left of Door"
- "TV is directly facing the Sofa, 3.0m away"
...
Answer:"""
    ...
    def vl_answer_choice(
        ...
        question_type: str,
        total_frames: int = 0,
        calibration: CalibrationResult = None,  # V7.2 新增
    ) -> Tuple[str, str]:
        """VL 推理 - 选择题 (方向、顺序、距离比较、路线规划)
        
        V7.2 增强：在 direction/rel_distance 任务中添加空间锚点描述
        """
        if self.vl_model is None:
        ...
        # V7.2: 为空间任务生成空间锚点
        spatial_anchors = ""
        if calibration and question_type in ('object_rel_direction_easy', 'object_rel_direction_medium', 
                                             'object_rel_direction_hard', 'object_rel_distance', 'route_planning'):
            spatial_anchors = self._build_spatial_anchors(mind_map, calibration)
...
=== SPATIAL ANCHORS (from 3D perception) ===
{spatial_anchors}
Note: These are relative positions from the perception system. Use them as hints but verify with the video."""
...
=== SPATIAL ANCHORS (from 3D perception) ===
{spatial_anchors}
Note: Distance values are calibrated estimates. Use them as hints but verify with the video."""
...
=== SPATIAL LAYOUT (from 3D perception) ===
{spatial_anchors}
Use this layout to reason about efficient routes."""
...
Answer:"""
    ...
    def _build_spatial_anchors(self, mind_map: Dict[str, MindMapEntity], 
                               calibration: CalibrationResult) -> str:
        """V7.2 新增：构建空间锚点描述
        
        将 3D 位置转换为自然语言空间关系描述，帮助 VL 模型理解空间布局。
        例如：
        - "Chair_1 is 1.5m behind the Table"
        - "Sofa is 2.3m to the left of Door"
        """
        if not mind_map:
            return ""
        ...
        # 找出有有效 3D 位置的实体
        positioned = [(label, ent) for label, ent in mind_map.items() 
                      if ent.position_3d is not None]
        ...
        # 优先使用校准物体作为参考
        if calibration.calibration_object and calibration.calibration_object != 'default':
            for label, ent in positioned:
                if calibration.calibration_object.lower() in label.lower():
                    ref_label = label
                    ref_ent = ent
                    break
        ...
        if ref_ent is None:
            def get_size(ent):
                if ent.size_3d is not None:
                    return ent.size_3d[0] * ent.size_3d[1]
                return ent.get_estimated_size()
            
            positioned.sort(key=lambda x: -get_size(x[1]))
            ref_label, ref_ent = positioned[0]
        
        ref_pos = ref_ent.position_3d
        scale = calibration.scale_factor if calibration.scale_factor > 0 else 1.0
        
        anchors = [f"Reference object: {ref_label} (at scene center)"]
        ...
        for label, ent in positioned:
            ...
            pos = ent.position_3d
            # 计算相对位置
            dx = (pos[0] - ref_pos[0]) * scale  # 左右 (x)
            dy = (pos[1] - ref_pos[1]) * scale  # 上下 (y) 
            dz = (pos[2] - ref_pos[2]) * scale  # 前后 (z/depth)
            
            dist = np.sqrt(dx**2 + dz**2)  # 水平距离
            
            # 确定主要方向
            directions = []
            if abs(dz) > 0.3:
                directions.append("in front of" if dz > 0 else "behind")
            if abs(dx) > 0.3:
                directions.append("to the right of" if dx > 0 else "to the left of")
            
            if not directions:
                directions.append("near")
            
            direction_str = " and ".join(directions)
            anchors.append(f"- {label}: {dist:.1f}m {direction_str} {ref_label}")
    ...
    for sample in tqdm(samples, desc=f"GPU {gpu_id}"):
        ...
        try:
            ...
            if question_type == 'object_counting':
                ...
                # V7.2: 传入 calibration 以生成空间锚点
                rule_pred, rule_reasoning = qa.rule_answer_choice(mind_map, question, options, question_type)
                if vl_loaded:
                    vl_pred, vl_reasoning = qa.vl_answer_choice(
                        question, options, video_path, mind_map, question_type, total_frames,
                        calibration=calibration  # V7.2: 添加 calibration 参数
                    )
