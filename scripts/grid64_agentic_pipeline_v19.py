#!/usr/bin/env python3
"""
256³ Grid Mind Map — Agentic Pipeline V19 (Spatial Cognitive Loop)

Core Idea — Closed-Loop Active Perception:
  Mental Map (3D Grid) provides structured spatial belief
  → guides Active Focusing (selects co-occurrence/temporal frames)
  → VL verifies on focused frames (independent, no CODER influence)
  → VL disagreement signals belief instability
  → Belief Correction (filter noisy detections) → re-select frames → VL re-verifies

架构:
  P0: Belief Construction — build Grid + CODER computation (hidden from VL)
  P1: Global Perception — VL independently on full video
  P2: Active Focusing — Grid guides frame selection → VL judges focused frames
      Converge: P1 == P2 → done
  P3: Belief Correction Loop — VL disagreement triggers:
      a) Filter high-variance detections from key entities
      b) Re-compute co-occurrence frames from corrected Grid
      c) VL re-verifies on NEW focused frames
      → Majority vote among 3 VL calls (all independent visual judgments)

关键设计原则:
  1. Grid NEVER tells VL the answer. Grid only guides WHAT FRAMES VL sees.
  2. VL is always independent — no CODER hypothesis shown in any prompt
  3. Grid's contribution is Active Focusing (proven: focused VL 60% > full VL 25% on diverge)
  4. Belief Correction is driven by VL disagreement (true closed loop)
  5. CODER only used for: auto-ADD missing entities, numerical tasks

Evolution from V17/V18:
  V17: VL + CODER independent → 3-ticket vote → CODER poisons vote (30% acc)
  V18: CODER hypothesis shown to VL → VL anchored on wrong CODER (42% wrong anchoring)
  V19: Grid guides frames only → VL independent → disagreement → belief correction → re-focus
"""
...
# ============================================================================
# V19 CORE: Independent-First + Hypothesis-as-Referee (Choice Tasks)
# ============================================================================

def v19_loop(ctx, max_rounds=3):
    """V19: Spatial Cognitive Loop — Grid-Guided Active Perception with Closed-Loop Correction.

    The story:
      Mental Map (Grid) provides structured 3D belief → guides Active Focusing (frame selection)
      → VL verifies on focused frames → VL disagreement signals belief instability
      → Belief Correction (filter noisy detections) → new frame selection → VL re-verifies

    P0: Belief Construction — build Grid, run CODER (hidden from VL)
    P1: Global Perception — VL independently judges on full video
    P2: Active Focusing — Grid selects co-occurrence/temporal frames → VL focused judgment
        Converge: P1 == P2 → done
    P3: Belief Correction Loop — VL disagreement drives:
        a) Filter high-variance detections from key entities
        b) Re-compute co-occurrence frames from corrected Grid
        c) VL re-verifies on NEW focused frames
        → If still no consensus, trust the focused perception (P2 or P3 focused)

    Key: Grid NEVER tells VL the answer. Grid only guides WHAT FRAMES VL sees.
    """
    rp = []
    ct = _auto_coder_type(ctx.question, ctx.options) or ''

    # Numerical tasks: proven V17 path
    if not ctx.options:
    ...
    # CODER computation (may trigger auto-ADD for missing entities)
    ...
    # ──── Phase 1: Global Perception (VL independent, full video) ────
    rp.append("[P1:vl_independent]")
    ...
    logger.info(f"  P1: VL(global)={vl_ans_1}")

    # ──── Phase 2: Active Focusing (Grid-guided frame selection → VL focused) ────
    rp.append("[P2:active_focus]")

    if is_temporal:
        focus_frames_1, focus_ents = _get_entity_union_frames(ctx.grid, ctx.question, ctx.options)
        focus_type = 'temporal'
    else:
        focus_frames_1, focus_ents = _get_cooccurrence_frames(ctx.grid, ctx.question, ctx.options)
        focus_type = 'cooccur'

    focused_pil = None
    p2_used_focused = False
    if focus_frames_1 and len(focus_frames_1) >= 2:
        max_ff = 12 if is_temporal else 8
        focused_pil = extract_focused_frames(ctx.video_path, focus_frames_1, max_frames=max_ff)
    ...
    if focused_pil and len(focused_pil) >= 2:
        if is_temporal:
            ...
            vl_focus_prompt = _build_vl_focused_prompt(ctx, focus_ents, len(focused_pil))

        vl_ans_2 = _clean(ctx.vl.call_with_frames(vl_focus_prompt, focused_pil, max_tokens=128), ctx)
        ctx.vl_calls += 1
        p2_used_focused = True
        ...
        # Fallback: VL with grid slice
        si = generate_grid_slice(ctx.grid, ctx.question, ctx.options)
        vl_ans_2 = _clean(ctx.vl.call(vl_prompt, ctx.video_path, max_tokens=128, images=[si]), ctx)
        ctx.vl_calls += 1
        rp.append(f"[vl_slice] {vl_ans_2}")
        logger.info(f"  P2: VL(slice)={vl_ans_2}")

    # ──── Phase 2 Convergence: Both VL agree → high confidence, done ────
    ...
    # ──── Phase 3: Belief Correction → Re-Focus → Re-Verify ────
    # VL disagreed with itself → the spatial evidence is ambiguous.
    # This signals that Grid's belief (entity positions) may be noisy.
    # Correct the belief, re-select frames, and let VL verify on NEW frames.
    rp.append("[P3:belief_correction]")
    logger.info(f"  P3: VL disagreed ({vl_ans_1} vs {vl_ans_2}), correcting belief")

    # 3a. Filter high-variance detections for key entities
    ...
    for name in rel_names:
        ...
        if pos_std > 0.3:  # Unstable detection → filter outlier frames
            ds = sorted(e.detections, key=lambda d: d.get('confidence', 0))
            nr = max(1, len(ds) // 3)
            bf = [d.get('frame_order', -1) for d in ds[:nr]]
            bf = [f for f in bf if f >= 0]
            if bf:
                ...
                if 'removed' in r:
                    ...
                    logger.info(f"  P3 belief correction: {r}")

    # 3b. Re-compute frame selection from corrected Grid
    if is_temporal:
        focus_frames_2, focus_ents_2 = _get_entity_union_frames(ctx.grid, ctx.question, ctx.options)
    else:
        focus_frames_2, focus_ents_2 = _get_cooccurrence_frames(ctx.grid, ctx.question, ctx.options)

    # Check if frame selection actually changed
    frames_changed = (set(focus_frames_2) != set(focus_frames_1)) if focus_frames_1 else False
    rp.append(f"[reframe] changed={frames_changed} n={len(focus_frames_2)}")
    logger.info(f"  P3: frames_changed={frames_changed}, "
                f"before={len(focus_frames_1)} after={len(focus_frames_2)}")

    # 3c. VL re-verifies on new focused frames
    vl_ans_3 = ''
    if focus_frames_2 and len(focus_frames_2) >= 2:
        max_ff = 12 if is_temporal else 8
        focused_pil_2 = extract_focused_frames(ctx.video_path, focus_frames_2, max_frames=max_ff)
        if focused_pil_2 and len(focused_pil_2) >= 2:
            if is_temporal:
                prompt_3 = _build_temporal_vl_prompt(ctx, focus_ents_2, len(focused_pil_2))
            else:
                prompt_3 = _build_vl_focused_prompt(ctx, focus_ents_2, len(focused_pil_2))
            vl_ans_3 = _clean(ctx.vl.call_with_frames(prompt_3, focused_pil_2, max_tokens=128), ctx)
            ctx.vl_calls += 1
            rp.append(f"[vl_refocused] {vl_ans_3} ({len(focused_pil_2)}f)")
            logger.info(f"  P3: VL(refocused)={vl_ans_3} ({len(focused_pil_2)} frames)")

    # ──── Final Decision: consensus among VL calls ────
    # All VL calls are independent visual judgments (no CODER influence).
    # Use majority vote among [VL_P1, VL_P2, VL_P3].
    # If no majority, trust the most focused perception.
    vl_votes = [v for v in [vl_ans_1, vl_ans_2, vl_ans_3] if v in 'ABCD']
    vote_counts = Counter(vl_votes)

    if vote_counts and vote_counts.most_common(1)[0][1] >= 2:
        ans = vote_counts.most_common(1)[0][0]
        rp.append(f"[vl_consensus] {dict(vote_counts)}→{ans}")
        logger.info(f"  Final: VL consensus {dict(vote_counts)} → {ans}")
    elif vl_ans_3 and vl_ans_3 in 'ABCD':
        # No majority → trust the latest focused perception (P3 refocused)
        ans = vl_ans_3
        rp.append(f"[trust_refocused] {ans}")
        logger.info(f"  Final: trust refocused VL → {ans}")
    elif p2_used_focused and vl_ans_2 in 'ABCD':
        # P3 didn't produce valid answer → trust P2 focused
        ans = vl_ans_2
        rp.append(f"[trust_focused] {ans}")
        logger.info(f"  Final: trust P2 focused VL → {ans}")
    else:
        ans = vl_ans_1 if vl_ans_1 in 'ABCD' else 'A'
...
class AgenticPipelineV19:
    ...
    def process_scene(self, video_path, questions, grid=None):
        ...
        for sample in questions:
            ...
            try:
                ans, reasoning = v19_loop(ctx, max_rounds=self.max_rounds)
            except Exception as e:
            ...
            results.append({
                ...
                'converged_phase': (2 if '[converge_p2]' in reasoning else 3),
...
def _print_summary(all_results, od, ts):
    print("\n" + "=" * 120)
    print("Agentic Pipeline V19 — Spatial Cognitive Loop (Closed-Loop Active Perception)")
    print(f"Architecture: P0(Belief) → P1(Global VL) → P2(Active Focus) → P2 converge / P3(Belief Correction → Re-Focus → Re-Verify)")
    ...
    print(f"  {'Task':<35} {'N':>4} {'V7':>6} {'V19':>6} {'Δ':>6}  {'VL':>4} {'Bel%':>4} {'Foc%':>4} {'AvgP':>4} {'T':>5}")
    print("-" * 110)
    for qt in tts:
        qr = [r for r in all_results if r['question_type'] == qt]
        v7 = np.mean([r.get('v7_vl_score', 0) for r in qr])
        v19 = np.mean([r['score'] for r in qr])
        d = v19 - v7; vl = np.mean([r.get('vl_calls', 0) for r in qr])
        ...
        print(f"  {qt:<35} {len(qr):>4} {v7:>5.3f} {v19:>5.3f} {d:>+5.3f}{mk} {vl:>4.1f} {bm:>3.0f}% {vf:>3.0f}% {avg_p:>3.1f} {tavg:>4.0f}s")

    ov7 = np.mean([r.get('v7_vl_score', 0) for r in all_results])
    ov19 = np.mean([r['score'] for r in all_results])
    print("-" * 110)
    print(f"  {'Overall':<35} {len(all_results):>4} {ov7:>5.3f} {ov19:>5.3f} {ov19-ov7:>+5.3f}")
    ...
    print(f"  Convergence: P2={p2} ({100*p2/max(1,len(all_results)):.1f}%), P3={p3} ({100*p3/max(1,len(all_results)):.1f}%)")
    print(f"  Belief Modified: {bm_n} ({100*bm_n/max(1,len(all_results)):.1f}%), VL Focused: {vf_n} ({100*vf_n/max(1,len(all_results)):.1f}%)")

    summary = {'timestamp': ts, 'version': 'v19_independent_first_referee', 'n_samples': len(all_results),
               'overall': {'v7': float(ov7), 'v19': float(ov19), 'delta': float(ov19 - ov7)},
               'avg_vl_calls': float(avl), 'avg_time_s': float(at),
               'convergence': {'p2': p2, 'p3': p3},
               ...
               'by_task': {qt: {'n': len([r for r in all_results if r['question_type'] == qt]),
                                ...
                                'v19': float(np.mean([r['score'] for r in all_results if r['question_type'] == qt]))}
                           for qt in tts}}
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="V19 Independent-First + Hypothesis-as-Referee Pipeline")
    ...
    pipe = AgenticPipelineV19(device=args.device, vl_model_path=vl_model,
    ...
    if args.gpu_id is not None:
        od = PROJECT_ROOT / "outputs" / "agentic_pipeline_v19_ref" / f"gpu{args.gpu_id}"
    else:
        od = PROJECT_ROOT / "outputs" / f"agentic_pipeline_v19_{timestamp}"
