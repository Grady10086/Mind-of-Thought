...
# ── Tool 2.5: route_verify_tool (Scheme 3) ───────────────────────────────────
...
def manager_code_agent_loop(ctx: ToolExecutionContext, max_steps: int = 4) -> Tuple[str, str]:
    ...
    for iteration in range(MAX_ITER):
        ...
        # 【Scheme 3】Step 5b: Route专用验证 — 对route任务使用route_verify_tool
        if coder_type == 'route' and ctx.options and iteration == 0:
            # 获取CODER推荐的选项
            m_coder_ans = re.search(r'answer=([A-D])', final_coder_result)
            coder_letter = m_coder_ans.group(1) if m_coder_ans else None
            
            # 验证CODER推荐的选项
            if coder_letter:
                verify_result_1 = route_verify_tool(ctx, coder_letter)
                reasoning_parts.append(f"[route_verify] {verify_result_1[:60]}")
                
                # 如果CODER推荐不可行，尝试验证其他选项
                if 'INFEASIBLE' in verify_result_1 or 'LOW' in verify_result_1:
                    other_options = [opt[0] for opt in ctx.options if opt[0] != coder_letter]
                    best_alternative = None
                    best_score = -1
                    
                    for opt_letter in other_options[:2]:  # 最多验证2个其他选项
                        verify_result_2 = route_verify_tool(ctx, opt_letter)
                        if 'FEASIBLE' in verify_result_2 and 'HIGH' in verify_result_2:
                            best_alternative = opt_letter
                            best_score = 2
                            break
                        elif 'FEASIBLE' in verify_result_2 and best_score < 1:
                            best_alternative = opt_letter
                            best_score = 1
                    
                    if best_alternative:
                        reasoning_parts.append(f"[route_override] CODER={coder_letter} → VL={best_alternative}")
                        # 修改final_coder_result以反映新的选择
                        final_coder_result = final_coder_result.replace(f"answer={coder_letter}", f"answer={best_alternative}")
                        final_coder_result += f" [route_verified_override: {best_alternative}]"
