#!/usr/bin/env python3
"""
64³ Grid Mind Map — Agentic Pipeline V7 (Self-Verification Evolving)

核心改进 (V6→V7):
  1. Self-Verification: CODER结果经VL Pairwise Verification二次校验
  2. Auto-ADD: CODER返回not-found时自动触发Evolutor ADD补全缺失实体
  3. Verified Confidence: CODER+VL验证一致时提升为"verified"信心，Decide阶段强信任
  4. VL Pairwise: rel_distance用VL逐对比较, direction用VL视觉方位确认

架构:
  Manager (VL Agent) ── Gather Step ──┬── critic_tool()      : VL审查Grid质量
                                       ├── evolutor_tool()    : 修正Grid (DELETE/ADD/REFINE/SCALE_ADJUST)
                                       ├── coder_tool()       : 确定性空间计算
                                       ├── grid_query_tool()  : 查询Grid数据
                                       └── final_answer()     : 提交最终答案
                       ── Verify Step ── VL Pairwise Verification (自动, 对选择题)
                       ── Decide Step ── VL 看视频做视觉推理（Grid参考按信心呈现）
...
def evolutor_tool(ctx: ToolExecutionContext, action: str, target: str, reason: str = "") -> str:
    ...
    修改 Grid。
    Args:
        action: "DELETE" (删除误检) / "ADD" (搜索并添加缺失entity) / "REFINE" (重新检测) / "SCALE_ADJUST" (调整Grid尺度)
        target: entity_id(DELETE) 或 entity_name(ADD/REFINE) 或 scale_factor(SCALE_ADJUST, e.g. "0.5" 缩小一半)
        reason: 修改原因
    Returns:
        操作结果
    """
    ...
    if action == 'DELETE':
        ...
        for e in old:
            del grid.entities[e.entity_id]
        if ctx.builder:
            added = ctx.builder.search_and_add_entity(grid, name)
            if added:
                phys = grid.grid_to_physical(added.grid_position)
                ctx.tool_trace.append({'tool': 'evolutor', 'action': 'REFINE', 'target': name, 'ok': True})
                return f"REFINE '{name}' → '{added.entity_id}' at ({phys[0]:.2f},{phys[1]:.2f},{phys[2]:.2f})m."
        ctx.tool_trace.append({'tool': 'evolutor', 'action': 'REFINE', 'target': name, 'ok': False})
        return f"REFINE failed: could not re-detect '{name}'."
...
def _coder_result_confidence(computation: str, result_str: str, grid: Grid64 = None) -> str:
    """评估coder计算结果的可信度 — 基于结果字符串信号 + Grid质量指标，不按computation类型区分"""
    r = result_str.lower()
    ...
    # route_sim: 检查score margin — 只有在最佳选项显著领先时才给normal
    if 'route_sim' in r:
        # 提取各选项scores
        import re as _re
        ...
        if len(scores) >= 2:
            ...
            if margin > 0.3 and scores_sorted[0] > 0:
                return 'normal'  # 高margin, 有信心
        return 'low'  # 低margin或无法解析, 不可靠

    # 检查是否触碰了clamp边界 (Grid计算中的硬限制，任何类型都适用)
    clamp_signals = [
        'answer=20.00m', 'answer=0.10m',    # distance clamp
        'answer=80', 'answer=5.0',           # room_size clamp
        'answer=200.0cm', 'answer=5.0cm',    # size clamp
    ]
    ...
    # 基于Grid质量指标的通用可信度评估
    # meters_per_grid (mpg) 反映了Grid的空间精度
    # mpg 越大，64格覆盖的物理空间越大，每格精度越低
    # 当结果涉及物理量数值（距离m/面积m²/尺寸cm）时，mpg较大意味着精度不够
    # 注意：仅对包含物理量数值的结果做此检查，选项类结果（如方向A/B/C/D）
    #       主要依赖角度关系（arctan2），受mpg影响较小
    if grid is not None:
        mpg = grid.meters_per_grid
        # 检查结果是否包含物理量数值（distance=X.XXm, area=XXm², size=XXcm）
...
def _self_verify(ctx: ToolExecutionContext, coder_type: str, coder_result: str,
    ...
    策略 (V7.1修正):
      - agree → 提升为 'verified' (Decide强信任CODER)
      - disagree/override → 保持原有confidence不变 (不降级，不跳过Decide)
      VL独立空间推理不可靠，只用VL来"确认"CODER，不用VL来"推翻"CODER
    
    Returns: (updated_confidence, verification_result, verify_log)
      - updated_confidence: 'verified' (一致) 或 原值不变
      - verification_result: 'agree', 'disagree', '' (无验证)
      - verify_log: 日志文本
    """
    ...
    if verify_result == 'agree':
        # CODER + VL 一致 → verified (最高信心)
        return 'verified', verify_result, verify_log
    else:
        # disagree / override → 不改变信心 (VL空间推理不可靠，不能推翻CODER)
        # 将override转为disagree（不走override路径）
...
def _build_gather_prompt(ctx: ToolExecutionContext) -> str:
    """Step 1 prompt: Manager看问题+Grid，选择要执行的工具组合 (JSON action list)"""
    grid_text = _grid_to_text(ctx.grid)
...
3. EVOLUTOR: Fix Grid errors (only after CRITIC finds issues)
   Actions: DELETE (remove wrong entity), ADD (search for missing entity), REFINE (re-detect), SCALE_ADJUST (adjust Grid scale with factor e.g. "0.5" or "auto")
...
def _build_decide_prompt(ctx: ToolExecutionContext, gathered_info: str,
    ...
    """统一的 Decide prompt — 选择题和数值题共用同一个函数，VL视觉推理为主
...
def _build_evolve_prompt(ctx: ToolExecutionContext, critic_result: str, gathered_info: str) -> str:
    """Step 1b prompt: Critic found issues → Manager decides whether to evolve"""
    options_text = "\n".join(ctx.options) if ctx.options else ""
...
- REFINE a wrong entity: {{"tool":"EVOLUTOR","action":"REFINE","target":"entity_name"}}
...
def manager_code_agent_loop(ctx: ToolExecutionContext, max_steps: int = 4) -> Tuple[str, str]:
    """
    Autonomous Manager Loop — V7 Self-Verification Evolving
    
    Architecture:
      Step 1:  Gather — Manager自主选择工具 (CODER/CRITIC) + 自动补充
      Step 1a: Auto-ADD — CODER返回not-found时自动ADD缺失实体 → 重算CODER
      Step 1b: Critic/Evolve — CRITIC发现issues → Manager决定是否EVOLVE → 重算CODER
      Step 1c: Self-Verify — VL pairwise 二次校验 CODER 结果 (verified/override/disagree)
      Step 2:  Decide — VL看视频做视觉推理，按信心级别呈现Grid参考
               如果 override → 直接使用VL答案，跳过Decide
    
    核心原则:
      - Manager自主决策，不依赖question_type
      - CODER not-found → 自动ADD补全 → 重算
      - CODER选择题结果 → VL pairwise verification → verified/override/disagree
      - verified → Decide强信任CODER | override → 跳过Decide直接用VL答案
    
    Returns: (prediction, reasoning_trace)
    """
    reasoning_parts = []

    # ── Step 1: Gather — Manager选择actions + 自动补充 ──
    ...
    logger.info(f"  Manager gather: {len(actions)} actions parsed from: {gather_response[:100]}")
    reasoning_parts.append(f"[gather] actions={json.dumps(actions, default=str)[:150]}")

    # 如果Manager没选任何CODER，自动推断一个（确保有计算参考）
    ...
    if not has_coder:
        ...
        if auto_type:
            ...
            reasoning_parts.append(f"[auto_coder] type={auto_type}")

    # 如果Manager没选CRITIC，自动对关键entities做检查（提升Grid质量）
    has_critic = any(a.get('tool', '').upper() == 'CRITIC' for a in actions)
    if not has_critic:
        entities = _extract_question_entities(ctx.question, ctx.options)
        if entities:
            # 只检查在Grid中能找到的entities
            found_names = []
            for name in entities[:3]:
                if ctx.grid.get_by_category(name):
    ...
    for line in gathered_info.split('\n'):
        if line.startswith('[CODER'):
            coder_result = line.split('] ', 1)[1] if '] ' in line else line
            coder_confidence = _coder_result_confidence(coder_type, coder_result, ctx.grid)
            break
    reasoning_parts.append(f"[coder_conf] type={coder_type}, conf={coder_confidence}")

    # ── Step 1a: Auto-ADD — CODER返回not-found时自动补全缺失实体 ──
    auto_add_modified = False
    if 'not found' in coder_result.lower() or 'not_found' in coder_result.lower():
        auto_add_modified, add_log = _auto_add_missing_entities(ctx, coder_result)
        if add_log:
            gathered_info += f"\n{add_log}"
            reasoning_parts.append(f"[auto_add] modified={auto_add_modified} {add_log[:80]}")
        # 如果 Grid 被修改了，重算 CODER
        if auto_add_modified:
            coder_actions = [a for a in actions if a.get('tool', '').upper() == 'CODER']
            if coder_actions:
                recompute = _execute_gather_actions(ctx, coder_actions)
    ...
    # ── Step 1b: 如果 Critic 发现 issues → evolve ──
    critic_results = [g for g in gathered_info.split('\n') if g.startswith('[CRITIC]')]
    has_issues = any('issue' in c.lower() and 'no issue' not in c.lower() for c in critic_results)

    if has_issues and critic_results:
        critic_text = "\n".join(critic_results)
        ...
        if evolve_actions:
            logger.info(f"  Manager evolve: {len(evolve_actions)} actions")
            evolve_result = _execute_gather_actions(ctx, evolve_actions)
            gathered_info += f"\n{evolve_result}"
            reasoning_parts.append(f"[evolve] {evolve_result[:100]}")

            # 如果 Grid 被修改, 重算 CODER
            if any(e.get('ok') for e in ctx.tool_trace if e.get('tool') == 'evolutor'):
                coder_actions = [a for a in actions if a.get('tool', '').upper() == 'CODER']
                if coder_actions:
                    recompute = _execute_gather_actions(ctx, coder_actions)
                    gathered_info += f"\n[RECOMPUTED]\n{recompute}"
    ...
    # ── Step 1c: Self-Verify — VL pairwise 二次校验 CODER 结果 ──
    verify_result = ''
    verify_log = ''
    if coder_result and coder_type:
        coder_confidence, verify_result, verify_log = _self_verify(
            ctx, coder_type, coder_result, coder_confidence)
        if verify_log:
            reasoning_parts.append(f"[verify] {verify_result} | {verify_log[:100]}")

    # ── Step 2: Decide — VL看视频做视觉推理 ──
    # verified → Decide prompt 强信任 CODER | disagree → 保持normal，不override
    decide_prompt = _build_decide_prompt(ctx, gathered_info, coder_confidence, coder_result,
...
class AgenticPipelineV5:
    """V5: CodeAgent-Style Self-Evolving Agentic System"""
    ...
    def process_sample(self, grid: Grid64, sample: Dict, video_path: str) -> Dict:
        ...
        return {
            ...
            'verify_override': '[decide_override]' in reasoning.lower(),
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="V7 Agentic Pipeline — Self-Verification Evolving")
    ...
    if args.gpu_id is not None:
        output_dir = PROJECT_ROOT / "outputs" / "agentic_pipeline_v7_full" / f"gpu{args.gpu_id}"
    else:
        output_dir = PROJECT_ROOT / "outputs" / f"agentic_pipeline_v7_{timestamp}"
...
def _print_summary(all_results, output_dir, timestamp):
    print("\n" + "=" * 140)
    print("Agentic Pipeline V7 — Self-Verification Evolving (Auto-ADD + VL Pairwise Verify + Verified Confidence)")
    print(f"Architecture: Manager(VL CodeAgent) ─ Gather → Auto-ADD → Critic/Evolve → Self-Verify → Decide")
    print(f"New: CODER not-found → auto ADD | VL pairwise verification → verified/override | Decide respects verified confidence")
    print(f"Baselines: V7 VL=63.61%, V6=65.48%  |  Samples: {len(all_results)}")
    print("=" * 140)

    task_types = sorted(set(r['question_type'] for r in all_results))

    print(f"\n{'Task':<35} {'N':>4} {'V7':>6} {'V7p':>6} {'Δ':>6}  {'VL#':>4} {'Evo%':>5} {'Cod%':>5} {'Add%':>5} {'Vfy%':>5} {'Ovr%':>5} {'t/s':>5}")
    ...
    for qt in task_types:
        ...
        ovr = np.mean([1 if r.get('verify_override') else 0 for r in qr]) * 100
        t_avg = np.mean([r.get('elapsed_s', 0) for r in qr])
        mk = "+" if d > 0.01 else ("-" if d < -0.01 else "=")
        print(f"  {qt:<35} {len(qr):>4} {v7:>5.3f} {v5:>5.3f} {d:>+5.3f}{mk} {vl:>4.1f} {evo:>4.0f}% {cod:>4.0f}% {add:>4.0f}% {vfy:>4.0f}% {ovr:>4.0f}% {t_avg:>4.0f}s")
        all_v7.extend([r['v7_vl_score'] for r in qr])
        all_v5.extend([r['score'] for r in qr])

    print(f"{'-'*35} {'-'*4} {'-'*6} {'-'*6} {'-'*6}")
    ov7, ov5 = np.mean(all_v7), np.mean(all_v5)
    print(f"  {'Overall':<35} {len(all_results):>4} {ov7:>5.3f} {ov5:>5.3f} {ov5-ov7:>+5.3f}")
    ...
    # V7 new feature stats
    n_add = sum(1 for r in all_results if r.get('auto_add_triggered'))
    n_vfy = sum(1 for r in all_results if r.get('verify_triggered'))
    n_ovr = sum(1 for r in all_results if r.get('verify_override'))
    print(f"  V7 features: Auto-ADD={n_add} ({100*n_add/len(all_results):.1f}%), "
          f"Verify={n_vfy} ({100*n_vfy/len(all_results):.1f}%), "
          f"Override={n_ovr} ({100*n_ovr/len(all_results):.1f}%)")
    ...
    if num:
        print(f"  Numerical: n={len(num)}, V7p={np.mean([r['score'] for r in num]):.3f}, V7={np.mean([r['v7_vl_score'] for r in num]):.3f}")
    if spa:
        print(f"  Spatial:   n={len(spa)}, V7p={np.mean([r['score'] for r in spa]):.3f}, V7={np.mean([r['v7_vl_score'] for r in spa]):.3f}")

    print(f"\nResults: {output_dir}")
    print(f"\n{'='*60}")
    print(f"  V7p Overall = {ov5:.4f}  vs  V7 VL = {ov7:.4f}  (Δ = {ov5-ov7:+.4f})")
    print(f"{'='*60}")

    summary = {
        'timestamp': timestamp,
        'version': 'v7_self_verification_evolving',
        'architecture': 'Manager(VL CodeAgent) → Gather → Auto-ADD → Critic/Evolve → Self-Verify → Decide',
        'design': 'Auto-ADD missing entities, VL pairwise verification, verified confidence, override path',
        'improvements': 'V6→V7: Self-Verify(pairwise), Auto-ADD(not-found), Verified Confidence, Override path',
        'n_samples': len(all_results),
        'overall': {'v7_vl': float(ov7), 'v7p': float(ov5), 'delta': float(ov5 - ov7)},
        'avg_vl_calls': float(avg_vl),
        'avg_time_s': float(avg_t),
        'v7_features': {
            'auto_add_count': n_add,
            'verify_count': n_vfy,
            'override_count': n_ovr,
        },
        'tool_usage': dict(tc),
        'by_task': {qt: {
            ...
            'v7p': float(np.mean([r['score'] for r in all_results if r['question_type'] == qt])),
            ...
            'override_rate': float(np.mean([1 if r.get('verify_override') else 0 for r in all_results if r['question_type'] == qt])),
        } for qt in task_types},
    }
