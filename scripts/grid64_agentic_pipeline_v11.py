#!/usr/bin/env python3
"""
256³ Grid Mind Map — Agentic Pipeline V11 (Autonomous Manager + V10 Decide)

核心改进 (V10→V11): Manager自主决策 + V10 Decide prompt完全复原
  1. 移除 _auto_select_coder_type 强制CODER注入 — Manager自主决定是否调用CODER
  2. 增强 Gather prompt — 每种CODER类型附带使用说明
  3. 恢复 _self_verify 的override→disagree转换（VL override不可靠）
  4. Single-Phase Decide (V10风格) — confidence-tiered呈现
     (Two-Phase实验证明有害: Phase 1引入bad anchor/锚定效应)
  5. Decide prompt与V10完全一致（含CRITIC展示、V10措辞）
  6. Soft fallback: distance+options→rel_distance（防止Manager选错type）

设计原则:
  - 自主CODER选择: Manager无强制注入，通过增强Gather prompt引导
  - V10 Decide prompt: verified强信任 / normal rough hint only / low视觉判断
  - distance+options自动纠正为rel_distance
...
def _self_verify(ctx: ToolExecutionContext, coder_type: str, coder_result: str,
    ...
    V11策略:
      - agree → 提升为 'verified'
      - override:X → 保留VL的答案X，传递给Decide（而非丢弃）
      - disagree → 保持原有confidence
    
    Returns: (updated_confidence, verification_result, verify_log)
      - verification_result可能是 'agree', 'disagree', 'override:X', '' (无验证)
    """
    ...
    if verify_result == 'agree':
        ...
        # V11修正: 恢复V10策略 — override转为disagree
        # VL verify对spatial tasks的override判断不可靠(rel_distance acc=34.7%)
        # 展示override信息会在Decide阶段引入矛盾信号，降低准确率
...
def manager_code_agent_loop(ctx: ToolExecutionContext, max_steps: int = 4) -> Tuple[str, str]:
    ...
    核心改进 (V10→V11):
      1. Manager自主决定是否调用CODER（不再强制注入）
      2. VL先独立判断(Phase 1)，再参考Grid建议(Phase 2)
      3. Grid结果中性呈现：只展示建议选项，不展示具体数值
      4. VL override答案保留，作为参考传递给Decide
    
    Architecture:
      Step 1: Gather — Manager看视频+Grid选择actions
      Step 2: CODER计算（如果Manager选择了的话）
      Step 3: Auto-ADD — 如果not found
      Step 4: Critic/Evolve — 如果发现问题
      Step 5: Self-Verify — VL验证CODER结果
      Step 6: Decide Phase 1 — VL独立判断（不看Grid）
      Step 7: Decide Phase 2 — VL参考Grid建议做最终决策
    """
    reasoning_parts = []
    MAX_ITER = 2
    ...
    # V11: 不再强制注入CODER — Manager的决策是最终决策
    # 但添加 soft fallback: 如果Manager选了distance但有options，纠正为rel_distance
    ...
    for act in actions:
        if act.get('tool', '').upper() == 'CODER':
            ...
            # Soft fallback: distance + options → rel_distance
            if coder_type == 'distance' and ctx.options:
                coder_type = 'rel_distance'
                act['type'] = 'rel_distance'
                reasoning_parts.append("[fix] distance→rel_distance (has options)")
            break
    ...
    for iteration in range(MAX_ITER):
        ...
        # V11修正: 先计算confidence，再传给verify，verify=agree时升级为verified
        ...
        if has_coder_result and final_coder_result and coder_type and ctx.options:
            ...
            final_coder_confidence = updated_conf  # agree → verified
            if verify_log:
    ...
    # ── Step 6+7: Decide ──
    # V11: 统一Single-Phase Decide (V10风格，confidence-tiered)
    # Two-Phase实验结论: Phase 1 VL独立判断引入bad anchor(锚定效应)
    # 对所有选择题(direction/appearance/rel_distance等)都有害
    # 
    # V11保留的改进: Manager自主CODER选择 + 增强Gather + VL override保留
...
class AgenticPipelineV11:
    """V11: Autonomous Unbiased Agentic System — VL-first, Grid-as-reference"""
    ...
    def process_sample(self, grid: Grid256, sample: Dict, video_path: str) -> Dict:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="V11 Agentic Pipeline — Autonomous Unbiased, VL-first")
    ...
    pipeline = AgenticPipelineV11(device=args.device, vl_model_path=vl_model, max_steps=args.max_steps)
    ...
    if args.gpu_id is not None:
        output_dir = PROJECT_ROOT / "outputs" / "agentic_pipeline_v11_full" / f"gpu{args.gpu_id}"
    else:
        output_dir = PROJECT_ROOT / "outputs" / f"agentic_pipeline_v11_{timestamp}"
...
def _print_summary(all_results, output_dir, timestamp):
    print("\n" + "=" * 140)
    print("Agentic Pipeline V11 — Autonomous Unbiased (VL-first, Grid-as-reference)")
    print(f"Architecture: Gather → [CODER if Manager chooses] → Verify → Phase1(VL independent) → Phase2(Grid reference)")
    print(f"V11 Features: No forced CODER | VL independent judgment first | Grid neutral reference | Override preserved")
    print(f"Baselines: V10 G256=66.73%, V7 VL=63.61%  |  Samples: {len(all_results)}")
    print("=" * 140)

    task_types = sorted(set(r['question_type'] for r in all_results))

    print(f"\n{'Task':<35} {'N':>4} {'V7':>6} {'V11':>6} {'Δ':>6}  {'VL#':>4} {'Evo%':>5} {'Cod%':>5} {'Add%':>5} {'Vfy%':>5} {'FF#':>4} {'t/s':>5}")
    ...
    for qt in task_types:
        ...
        v11 = np.mean([r['score'] for r in qr])
        d = v11 - v7
        ...
        print(f"  {qt:<35} {len(qr):>4} {v7:>5.3f} {v11:>5.3f} {d:>+5.3f}{mk} {vl:>4.1f} {evo:>4.0f}% {cod:>4.0f}% {add:>4.0f}% {vfy:>4.0f}% {ff:>4.1f} {t_avg:>4.0f}s")
        all_v7.extend([r['v7_vl_score'] for r in qr])
        all_v11.extend([r['score'] for r in qr])

    print(f"{'-'*35} {'-'*4} {'-'*6} {'-'*6} {'-'*6}")
    ov7, ov11 = np.mean(all_v7), np.mean(all_v11)
    print(f"  {'Overall':<35} {len(all_results):>4} {ov7:>5.3f} {ov11:>5.3f} {ov11-ov7:>+5.3f}")
    ...
    print(f"  V11 features: Auto-ADD={n_add} ({100*n_add/len(all_results):.1f}%), "
    ...
    if num:
        print(f"  Numerical: n={len(num)}, V11={np.mean([r['score'] for r in num]):.3f}, V7={np.mean([r['v7_vl_score'] for r in num]):.3f}")
    if spa:
        print(f"  Spatial:   n={len(spa)}, V11={np.mean([r['score'] for r in spa]):.3f}, V7={np.mean([r['v7_vl_score'] for r in spa]):.3f}")

    print(f"\nResults: {output_dir}")
    print(f"\n{'='*60}")
    print(f"  V11 Overall = {ov11:.4f}  vs  V7 VL = {ov7:.4f}  (Δ = {ov11-ov7:+.4f})")
    print(f"{'='*60}")

    summary = {
        'timestamp': timestamp,
        'version': 'v11_autonomous_unbiased',
        'architecture': 'Gather → [CODER if Manager] → Verify → Phase1(VL) → Phase2(Grid ref)',
        'design': 'No forced CODER | VL independent first | Grid neutral reference | Override preserved',
        'improvements': 'V10→V11: Remove forced CODER, VL-first 2-phase decide, neutral Grid presentation, preserve VL override',
        'n_samples': len(all_results),
        'overall': {'v7_vl': float(ov7), 'v11': float(ov11), 'delta': float(ov11 - ov7)},
        ...
        'by_task': {qt: {
            ...
            'v11': float(np.mean([r['score'] for r in all_results if r['question_type'] == qt])),
