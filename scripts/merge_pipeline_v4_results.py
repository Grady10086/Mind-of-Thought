...
SHARD_DIR = PROJECT_ROOT / "outputs" / "agentic_pipeline_v4_full"
...
def main():
    ...
    print("\n" + "=" * 130)
    print("Agentic Pipeline V4 FULL TEST (ADD Evolution + Position3D + 分难度Direction)")
    print(f"架构: Manager → Critic(16帧) → Manager(DELETE/ADD) → Evolutor(DELETE+ADD) → Reasoner")
    print(f"Direction: easy=Grid优先, medium/hard=VL优先 | Counting: 8帧+完整Grid | 空间型: 16帧VL")
    print(f"基准: V7 VL Overall = 63.61%")
    print(f"测试样本: {len(all_results)}")
    print("=" * 130)

    print(f"\n{'Task':<35} {'N':>5} {'V7_VL':>7} {'V4':>7} {'Delta':>7} {'Issues':>7} {'Evolve%':>8}")
    ...
    print(f"\n  数值型: n={len(numerical)}, V4={np.mean([r['score'] for r in numerical]):.3f}, V7={np.mean([r['v7_vl_score'] for r in numerical]):.3f}")
    print(f"  空间型: n={len(spatial)}, V4={np.mean([r['score'] for r in spatial]):.3f}, V7={np.mean([r['v7_vl_score'] for r in spatial]):.3f}")
    ...
    if with_issues:
        ...
        print(f"    V4={np.mean([r['score'] for r in with_issues]):.3f} V7={np.mean([r['v7_vl_score'] for r in with_issues]):.3f}")
    if no_issues:
        print(f"  No critic issues (n={len(no_issues)}):")
        print(f"    V4={np.mean([r['score'] for r in no_issues]):.3f} V7={np.mean([r['v7_vl_score'] for r in no_issues]):.3f}")

    # Evolution效果 (分DELETE和ADD)
    if evolve_total > 0:
        ...
        # ADD vs DELETE breakdown
        add_results = [r for r in evo_results if any('ADD' in a for a in r.get('evolution_actions', []))]
        del_results = [r for r in evo_results if any('DELETE' in a for a in r.get('evolution_actions', []))]
        if add_results:
            add_better = sum(1 for r in add_results if r['score'] > r['v7_vl_score'] + 0.05)
            add_worse = sum(1 for r in add_results if r['score'] < r['v7_vl_score'] - 0.05)
            print(f"    ADD only (n={len(add_results)}): Better={add_better}, Worse={add_worse}")
        if del_results:
            del_better = sum(1 for r in del_results if r['score'] > r['v7_vl_score'] + 0.05)
            del_worse = sum(1 for r in del_results if r['score'] < r['v7_vl_score'] - 0.05)
            print(f"    DELETE only (n={len(del_results)}): Better={del_better}, Worse={del_worse}")

    # Direction策略分析
    print(f"\n{'='*60}")
    print("Direction Strategy Analysis (V4: easy=Grid, medium/hard=VL):")
    for qt in task_types:
        ...
        vl_preferred = sum(1 for r in qt_r if 'vl_preferred' in r.get('reasoning', ''))
        vl_fallback = sum(1 for r in qt_r if 'vl_fallback' in r.get('reasoning', ''))
        ...
        vl_pref_correct = sum(1 for r in qt_r if 'vl_preferred' in r.get('reasoning', '') and r['score'] > 0.5)
        fb_correct = sum(1 for r in qt_r if 'vl_fallback' in r.get('reasoning', '') and r['score'] > 0.5)
        print(f"  {qt}:")
        print(f"    agree={grid_vl_agree}({agree_correct}✓), grid_pref={grid_preferred}({prefer_correct}✓), vl_pref={vl_preferred}({vl_pref_correct}✓), vl_fb={vl_fallback}({fb_correct}✓)")
    ...
    summary = {
        ...
        'architecture': 'Manager → Critic(diagnose) → Manager(DELETE/ADD) → Evolutor(DELETE+ADD) → Reasoner',
    ...
    print(f"\n{'='*60}")
    print(f"FINAL: V4 Overall={op:.4f} vs V7 VL={ov7:.4f} (Delta={op-ov7:+.4f})")
    print(f"{'='*60}")
