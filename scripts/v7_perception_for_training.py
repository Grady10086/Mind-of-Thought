...
def worker_process(gpu_id: int, samples: List[Dict], vl_model_path: str, adapter_path: str, result_queue: mp.Queue):
    """GPU Worker 进程"""
...
def main():
    ...
    for gpu_id in range(num_gpus):
        p = mp.Process(
            ...
            args=(gpu_id, gpu_samples[gpu_id], args.vl_model, args.adapter_path, result_queue)
        )
        p.start()
        processes.append(p)
