#!/usr/bin/env python3
"""
256³ Grid Mind Map — Agentic Pipeline V13 (True Closed-Loop MoT)

核心改进 (V12→V13): 真正的 Act-Critique-Evolve 闭环
  V10-V12的问题: Critic的发现被丢弃，Evolve修的是不相关entity，MoT空转
  V13修复这个根本问题:

  1. 去掉无效的Critic→FILTER_FRAMES空转(V12中60%的case白做)
  2. Verify disagree → 提取CODER涉及的entity → 针对性FILTER_FRAMES修复
  3. 增加VL独立判断(Phase 1): CODER不可靠时让VL不看Grid独立判断
  4. MAX_ITER=3: 真正的闭环迭代有意义了
  5. Decide: confidence-aware + 3-vote Self-Consistency保留

  关键洞察: V12错V7对的有424个样本(8.3%)
    只需救回142个(2.8%)就能到70%
    核心是: 当CODER错时, 让VL自主判断（不受Grid误导）
...
class AgenticPipelineV13:
    """V13: True Closed-Loop MoT — VL Independent + Targeted FILTER + Dual-Track Decide"""
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="V13 Agentic Pipeline — True Closed-Loop MoT")
    ...
    pipeline = AgenticPipelineV13(device=args.device, vl_model_path=vl_model, max_steps=args.max_steps)
    ...
    if args.gpu_id is not None:
        output_dir = PROJECT_ROOT / "outputs" / "agentic_pipeline_v13_full" / f"gpu{args.gpu_id}"
    else:
        output_dir = PROJECT_ROOT / "outputs" / f"agentic_pipeline_v13_{timestamp}"
...
def _print_summary(all_results, output_dir, timestamp):
    print("\n" + "=" * 140)
    print("Agentic Pipeline V13 — True Closed-Loop MoT")
    print(f"Architecture: VL-Only(Phase A) → Gather → [CODER] → Verify → [Targeted FILTER] → Decide(Dual-Track)")
    print(f"V13 Features: VL independent judgment | Targeted FILTER | Dual-track Decide | 3-vote Self-Consistency")
    print(f"Baselines: V12=66.70%, V10 G256=66.73%, V7 VL=63.61%  |  Samples: {len(all_results)}")
    print("=" * 140)

    task_types = sorted(set(r['question_type'] for r in all_results))

    print(f"\n{'Task':<35} {'N':>4} {'V7':>6} {'V13':>6} {'Δ':>6}  {'VL#':>4} {'Evo%':>5} {'Cod%':>5} {'Add%':>5} {'Vfy%':>5} {'FF#':>4} {'t/s':>5}")
    ...
    for qt in task_types:
        ...
        v13 = np.mean([r['score'] for r in qr])
        d = v13 - v7
        ...
        print(f"  {qt:<35} {len(qr):>4} {v7:>5.3f} {v13:>5.3f} {d:>+5.3f}{mk} {vl:>4.1f} {evo:>4.0f}% {cod:>4.0f}% {add:>4.0f}% {vfy:>4.0f}% {ff:>4.1f} {t_avg:>4.0f}s")
        all_v7.extend([r['v7_vl_score'] for r in qr])
        all_v13.extend([r['score'] for r in qr])

    print(f"{'-'*35} {'-'*4} {'-'*6} {'-'*6} {'-'*6}")
    ov7, ov13 = np.mean(all_v7), np.mean(all_v13)
    print(f"  {'Overall':<35} {len(all_results):>4} {ov7:>5.3f} {ov13:>5.3f} {ov13-ov7:>+5.3f}")
    ...
    print(f"  V13 features: Auto-ADD={n_add} ({100*n_add/len(all_results):.1f}%), "
    ...
    if num:
        print(f"  Numerical: n={len(num)}, V13={np.mean([r['score'] for r in num]):.3f}, V7={np.mean([r['v7_vl_score'] for r in num]):.3f}")
    if spa:
        print(f"  Spatial:   n={len(spa)}, V13={np.mean([r['score'] for r in spa]):.3f}, V7={np.mean([r['v7_vl_score'] for r in spa]):.3f}")

    print(f"\nResults: {output_dir}")
    print(f"\n{'='*60}")
    print(f"  V13 Overall = {ov13:.4f}  vs  V7 VL = {ov7:.4f}  (Δ = {ov13-ov7:+.4f})")
    print(f"{'='*60}")

    summary = {
        'timestamp': timestamp,
        'version': 'v13_true_closed_loop_mot',
        'architecture': 'VL-Only(Phase A) → Gather → [CODER] → Verify → [Targeted FILTER] → Decide(Dual-Track)',
        'design': 'VL independent judgment | Targeted FILTER | Dual-track Decide | 3-vote Self-Consistency',
        'improvements': 'V12→V13: Remove broken Critic→Evolve, add VL Phase A, targeted FILTER on question entities, dual-track Decide',
        'n_samples': len(all_results),
        'overall': {'v7_vl': float(ov7), 'v13': float(ov13), 'delta': float(ov13 - ov7)},
        ...
        'by_task': {qt: {
            ...
            'v13': float(np.mean([r['score'] for r in all_results if r['question_type'] == qt])),
