#!/usr/bin/env python3
"""
256³ Grid Mind Map — Agentic Pipeline V20 (Iterative Self-Evolution)

Core Idea — True Closed-Loop Iterative Evolution:
...
V19 只做了一次修正就结束了（open-loop single correction）。
V20 将修正→重选帧→VL重看 变成真正的迭代循环：
  - 每轮迭代，Grid 的 belief 在进化（过滤阈值逐步收紧）
  - 每轮选出的帧必须和上一轮不同（否则提前终止，因为没有新信息）
  - 累积所有轮次的 VL 回答做加权投票（近期轮次权重更高）

架构:
  P0: Belief Construction — build Grid + CODER computation (hidden from VL)
  Loop (max_rounds iterations):
    P1: Active Focusing — Grid guides frame selection → VL judges focused frames
    Converge check: 与上一轮 VL 一致 → done
    Evolve: Belief Correction → 过滤高方差/低置信度检测 → Grid 进化
    New info check: 帧集合是否变化？没变 → 终止（无新信息）
  Final: 累积投票 or trust latest focused
...
关键设计原则:
  ...
  3. 每轮 Grid evolve 后选出不同的帧 → VL 获得新视觉信息 → 真正的新证据
  4. 收紧的过滤阈值: round1 std>0.4, round2 std>0.3, round3 std>0.2
  5. 迭代终止条件: (a) VL收敛 (b) 帧没变化 (c) 达到max_rounds

Evolution from V19:
  V19: P0 → P1(global) → P2(focused) → P3(single correction) → vote → done
  V20: P0 → [P1(global)] → Loop{ focused_VL → evolve Grid → new frames → focused_VL → ... }
"""
...
# ============================================================================
# V20 CORE: Iterative Self-Evolution Loop (Choice Tasks)
# ============================================================================
...
def _vl_on_frames(ctx, frames, ents, ftype, is_temporal, rp, round_id):
    """Run VL on focused frames. Returns (answer, frames_pil)."""
...
def v20_loop(ctx, max_rounds=3):
    """V20: Iterative Self-Evolution — Grid evolves each round, VL sees new frames each time.

    P0: Belief Construction — build Grid, run CODER (hidden from VL)
    P1: Global Perception — VL independently judges on full video (anchor)
    Loop (up to max_rounds):
      Select frames from current Grid → VL judges focused frames
      Convergence check (dual criterion):
        (a) Global consensus: focused VL == P1 global VL → strongest signal, done
        (b) Evolution stability: focused VL == previous focused VL → belief stabilized, done
      Evolve Grid (filter noisy detections, threshold tightens each round)
      If frames didn't change → terminate (no new info)
    Final: weighted vote with global anchor

    Key design:
      - P1 (global) serves as ANCHOR throughout — every round checks against it
      - Convergence = focused perception aligns with global perception (strongest)
      - Evolution stability = focused perception stabilizes across rounds (secondary)
      - Grid NEVER tells VL the answer. Grid only guides WHAT FRAMES VL sees.
    """
    rp = []
    ...
    # ──── P1: Global Perception (VL independent, full video) — serves as ANCHOR ────
    ...
    logger.info(f"  P1: VL(global)={vl_global}  ← anchor")

    # ──── Iterative Evolution Loop ────
    std_thresholds = [0.4, 0.3, 0.2, 0.15, 0.1]

    vl_history = [('global', vl_global)]  # (source, answer)
    prev_frames = set()
    prev_focused_answer = None  # Track focused VL answers for stability check
    converge_type = None
    n_rounds = 0
    ...
    for round_id in range(1, max_rounds + 1):
        ...
        # 2. New info check: if frames unchanged, no new visual evidence
        ...
        # 3. VL judges on focused frames
        vl_ans, _ = _vl_on_frames(ctx, frames, ents, ftype, is_temporal, rp, round_id)
        vl_history.append((f'R{round_id}', vl_ans))
        n_rounds = round_id

        # 4. Dual convergence check
        # (a) Global consensus: focused agrees with P1 anchor
        if vl_ans == vl_global and vl_ans in 'ABCD':
            converge_type = 'global_consensus'
            rp.append(f"[R{round_id}:global_consensus] focused==global={vl_ans}")
            logger.info(f"    R{round_id}: GLOBAL CONSENSUS focused==global={vl_ans}")
            break
        ...
        if prev_focused_answer is not None and vl_ans == prev_focused_answer and vl_ans in 'ABCD':
            ...
            rp.append(f"[R{round_id}:evolution_stable] focused_stable={vl_ans}")
    ...
    # ──── Final Decision ────
    focused_votes = [(ans, i) for i, (src, ans) in enumerate(vl_history) if src != 'global' and ans in 'ABCD']
    all_valid = [(ans, i) for i, (src, ans) in enumerate(vl_history) if ans in 'ABCD']

    if converge_type == 'global_consensus':
        # Strongest: global and focused agree → use it
        ans = vl_global
        rp.append(f"[final:global_consensus_R{n_rounds}] {ans}")
        logger.info(f"  Final: global consensus at R{n_rounds} → {ans}")

    elif converge_type == 'evolution_stable':
        # Focused VL stabilized but disagrees with global
        # Trust focused (it has evolved, cleaner evidence) but note the disagreement
        ...
        # No convergence → weighted vote with global anchor
        # Global gets weight = max_rounds (it sees the full video, always relevant)
        # Each focused round gets weight = round_id (later = cleaner Grid)
        weighted_counts = Counter()
        for src_ans, idx in all_valid:
            src = vl_history[idx][0]
            if src == 'global':
                w = max_rounds  # Global anchor: high weight
            else:
                # Extract round number from 'R1', 'R2', etc.
                w = int(src[1:]) + 1  # R1=2, R2=3, R3=4
            weighted_counts[src_ans] += w

        ans = weighted_counts.most_common(1)[0][0]
        rp.append(f"[final:weighted_vote] {dict(weighted_counts)}→{ans}")
        logger.info(f"  Final: weighted vote {dict(weighted_counts)} → {ans}")
...
class AgenticPipelineV20:
    ...
    def process_scene(self, video_path, questions, grid=None):
        ...
        for sample in questions:
            ...
            try:
                ans, reasoning = v20_loop(ctx, max_rounds=self.max_rounds)
            except Exception as e:
            ...
            results.append({
                ...
                'converge_type': ('global_consensus' if 'global_consensus' in reasoning
                                  else ('evolution_stable' if 'evolution_stable' in reasoning
                                  else ('weighted_vote' if 'weighted_vote' in reasoning
                                  else 'early'))),
                'converged': 'global_consensus' in reasoning or 'evolution_stable' in reasoning,
...
def _print_summary(all_results, od, ts):
    print("\n" + "=" * 120)
    print("Agentic Pipeline V20 — Iterative Self-Evolution (True Closed-Loop)")
    print(f"Architecture: P0(Belief) → P1(Global VL) → Loop{{ Focus → Evolve → Re-Focus → ... }}")
    ...
    print(f"  {'Task':<35} {'N':>4} {'V7':>6} {'V20':>6} {'Δ':>6}  {'VL':>4} {'Bel%':>4} {'Foc%':>4} {'AvgR':>4} {'T':>5}")
    print("-" * 110)
    for qt in tts:
        qr = [r for r in all_results if r['question_type'] == qt]
        v7 = np.mean([r.get('v7_vl_score', 0) for r in qr])
        v20 = np.mean([r['score'] for r in qr])
        d = v20 - v7; vl = np.mean([r.get('vl_calls', 0) for r in qr])
        ...
        print(f"  {qt:<35} {len(qr):>4} {v7:>5.3f} {v20:>5.3f} {d:>+5.3f}{mk} {vl:>4.1f} {bm:>3.0f}% {vf:>3.0f}% {avg_r:>3.1f} {tavg:>4.0f}s")

    ov7 = np.mean([r.get('v7_vl_score', 0) for r in all_results])
    ov20 = np.mean([r['score'] for r in all_results])
    print("-" * 110)
    print(f"  {'Overall':<35} {len(all_results):>4} {ov7:>5.3f} {ov20:>5.3f} {ov20-ov7:>+5.3f}")
    ...
    summary = {'timestamp': ts, 'version': 'v20_iterative_self_evolution', 'n_samples': len(all_results),
               'overall': {'v7': float(ov7), 'v20': float(ov20), 'delta': float(ov20 - ov7)},
               ...
               'by_task': {qt: {'n': len([r for r in all_results if r['question_type'] == qt]),
                                ...
                                'v20': float(np.mean([r['score'] for r in all_results if r['question_type'] == qt]))}
                           for qt in tts}}
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="V20 Iterative Self-Evolution Pipeline")
    ...
    if args.gpu_id is not None:
        od = PROJECT_ROOT / "outputs" / "agentic_pipeline_v20_ref" / f"gpu{args.gpu_id}"
    else:
        od = PROJECT_ROOT / "outputs" / f"agentic_pipeline_v20_{timestamp}"
