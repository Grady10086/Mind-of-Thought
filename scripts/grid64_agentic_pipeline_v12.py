#!/usr/bin/env python3
"""
256³ Grid Mind Map — Agentic Pipeline V12 (Self-Consistency Decide)

核心改进 (V11→V12): Self-Consistency投票 + 任务自适应confidence
  1. Decide阶段3次采样投票(Self-Consistency): temperature=0.7 sampling
  2. 对CODER低准确率类型(rel_distance/dir_hard)强制low confidence
     - rel_dist: CODER acc~36%, verify无区分力 → low, 让VL纯视觉判断
     - direction: 对于hard/medium, verify=agree反而更差 → 不升级为verified
  3. 仅对选择题投票，数值题保持greedy
  4. 继承V11的所有基础改进(Manager自主决策/增强Gather/V10 Decide)

理论估算:
  3-vote仅对p>50%的任务: 71.0% (vs V11 66.8%)
  3-vote全量: 69.1% (因为p<50%任务投票会更差)
  → 方案: 对p>50%任务投票, 对p<50%任务用low confidence
...
def manager_code_agent_loop(ctx: ToolExecutionContext, max_steps: int = 4) -> Tuple[str, str]:
    ...
    # ── Step 6+7: Decide ──
    # V12: Self-Consistency投票 + 任务自适应confidence
    # 对选择题(accuracy>50%的类型受益): 3次采样投票取多数
    # 对数值题: 保持greedy
    ...
    # V12: Self-Consistency — 对选择题做3次采样投票
    if ctx.options:
        ...
        if votes:
            ...
            reasoning_parts.append(f"[vote:{len(votes)}] {dict(vote_counts)} → {final_answer_str}")
        else:
            # fallback: greedy单次
...
class AgenticPipelineV12:
    """V12: Self-Consistency Decide + Adaptive Confidence"""
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="V12 Agentic Pipeline — Self-Consistency Decide")
    ...
    pipeline = AgenticPipelineV12(device=args.device, vl_model_path=vl_model, max_steps=args.max_steps)
    ...
    if args.gpu_id is not None:
        output_dir = PROJECT_ROOT / "outputs" / "agentic_pipeline_v12_full" / f"gpu{args.gpu_id}"
    else:
        output_dir = PROJECT_ROOT / "outputs" / f"agentic_pipeline_v12_{timestamp}"
...
def _print_summary(all_results, output_dir, timestamp):
    print("\n" + "=" * 140)
    print("Agentic Pipeline V12 — Self-Consistency Decide + Adaptive Confidence")
    print(f"Architecture: Gather → [CODER] → Verify(selective) → Decide(3-vote Self-Consistency)")
    print(f"V12 Features: Self-Consistency voting | Task-adaptive confidence | Selective verify upgrade")
    ...
    print(f"\n{'Task':<35} {'N':>4} {'V7':>6} {'V12':>6} {'Δ':>6}  {'VL#':>4} {'Evo%':>5} {'Cod%':>5} {'Add%':>5} {'Vfy%':>5} {'FF#':>4} {'t/s':>5}")
    ...
    for qt in task_types:
        ...
        v12 = np.mean([r['score'] for r in qr])
        d = v12 - v7
        ...
        print(f"  {qt:<35} {len(qr):>4} {v7:>5.3f} {v12:>5.3f} {d:>+5.3f}{mk} {vl:>4.1f} {evo:>4.0f}% {cod:>4.0f}% {add:>4.0f}% {vfy:>4.0f}% {ff:>4.1f} {t_avg:>4.0f}s")
        all_v7.extend([r['v7_vl_score'] for r in qr])
        all_v12.extend([r['score'] for r in qr])

    print(f"{'-'*35} {'-'*4} {'-'*6} {'-'*6} {'-'*6}")
    ov7, ov12 = np.mean(all_v7), np.mean(all_v12)
    print(f"  {'Overall':<35} {len(all_results):>4} {ov7:>5.3f} {ov12:>5.3f} {ov12-ov7:>+5.3f}")
    ...
    print(f"  V12 features: Auto-ADD={n_add} ({100*n_add/len(all_results):.1f}%), "
    ...
    if num:
        print(f"  Numerical: n={len(num)}, V12={np.mean([r['score'] for r in num]):.3f}, V7={np.mean([r['v7_vl_score'] for r in num]):.3f}")
    if spa:
        print(f"  Spatial:   n={len(spa)}, V12={np.mean([r['score'] for r in spa]):.3f}, V7={np.mean([r['v7_vl_score'] for r in spa]):.3f}")

    print(f"\nResults: {output_dir}")
    print(f"\n{'='*60}")
    print(f"  V12 Overall = {ov12:.4f}  vs  V7 VL = {ov7:.4f}  (Δ = {ov12-ov7:+.4f})")
    print(f"{'='*60}")

    summary = {
        'timestamp': timestamp,
        'version': 'v12_self_consistency_decide',
        'architecture': 'Gather → [CODER] → Verify(selective) → Decide(3-vote Self-Consistency)',
        'design': 'Self-Consistency voting | Task-adaptive confidence | Selective verify upgrade',
        'improvements': 'V11→V12: Self-Consistency 3-vote for choice Qs, rel_dist forced low, selective verify upgrade',
        'n_samples': len(all_results),
        'overall': {'v7_vl': float(ov7), 'v12': float(ov12), 'delta': float(ov12 - ov7)},
        ...
        'by_task': {qt: {
            ...
            'v12': float(np.mean([r['score'] for r in all_results if r['question_type'] == qt])),
