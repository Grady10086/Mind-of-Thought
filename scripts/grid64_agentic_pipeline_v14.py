#!/usr/bin/env python3
"""
256³ Grid Mind Map — Agentic Pipeline V14 (Type-Selective Bypass)

核心改进 (V13→V14): 类型自适应双轨策略

  关键发现:
    - 数值题: Pipeline (Grid/CODER) 大幅超越 QwenVL (+25pp), 必须保留
    - 选择题: Pipeline 反而伤害选择题 (-13pp vs QwenVL), 必须绕过
    - 根因: 复杂prompt+Grid/CODER信息误导VL的选择题判断
    - QwenVL官方benchmark用极简prompt, 选择题表现更好
...
def main():
    ...
    if args.gpu_id is not None:
        output_dir = PROJECT_ROOT / "outputs" / "agentic_pipeline_v14_full" / f"gpu{args.gpu_id}"
    else:
