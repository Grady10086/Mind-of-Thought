#!/usr/bin/env python3
"""
128³ Grid Mind Map — Agentic Pipeline V9 (Temporal Filtering)

核心改进 (V5→V9):
  1. Grid128: 从64³升级到128³，更精细的空间分辨率
  2. Manager自主读取Grid: 精简Grid展示，只显示与问题相关的entity子集
  3. 时间维度Evolve: 新增FILTER_FRAMES动作——剔除异常帧观测
  4. 弱化绝对距离，强化相对拓扑: 数值型任务(distance/size/room_size)默认标low信心

架构:
  Manager (VL Agent) ── Gather Step ──┬── critic_tool()      : VL审查Grid质量
                                       ├── evolutor_tool()    : 修正Grid (DELETE/ADD/FILTER_FRAMES/SCALE_ADJUST)
                                       ├── coder_tool()       : 确定性空间计算
                                       ├── grid_query_tool()  : 查询Grid数据
                                       └── final_answer()     : 提交最终答案
                       ── Verify Step ── VL Pairwise Verification
                       ── Decide Step ── VL看视频做视觉推理
...
def _coder_result_confidence(computation: str, result_str: str, grid: Grid128 = None) -> str:
    ...
    - 空间拓扑型(direction/rel_distance/route/appearance/count): 保持normal/verified
    """
    r = result_str.lower()
...
class AgenticPipelineV9:
    """V9: 128³ Grid with Temporal Filtering Evolving System"""
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="V9 Agentic Pipeline — 128³ Grid with Temporal Filtering")
    ...
    pipeline = AgenticPipelineV9(device=args.device, vl_model_path=vl_model, max_steps=args.max_steps)
    ...
    for si, scene_name in enumerate(my_scenes):
        ...
        try:
            ...
            for r in results:
                ...
                logger.info(
                    f"  {r['question_type'][:25]:25s} [VL:{r['vl_calls']} {evo}{cod}{ff}] "
    ...
    if args.gpu_id is not None:
        output_dir = PROJECT_ROOT / "outputs" / "agentic_pipeline_v9_full" / f"gpu{args.gpu_id}"
    else:
        output_dir = PROJECT_ROOT / "outputs" / f"agentic_pipeline_v9_{timestamp}"
...
def _print_summary(all_results, output_dir, timestamp):
    print("\n" + "=" * 140)
    print("Agentic Pipeline V9 — 128³ Grid with Temporal Filtering")
    print(f"Architecture: Manager(VL CodeAgent) ─ Gather → Auto-ADD → Critic/Evolve(FILTER_FRAMES) → Self-Verify → Decide")
    print(f"V9 Features: Grid128 + Focused Grid Display + Temporal Filter + Weakened Numerical Confidence")
    print(f"Baselines: V7 VL=63.61%, V5=64.09%  |  Samples: {len(all_results)}")
    print("=" * 140)

    task_types = sorted(set(r['question_type'] for r in all_results))

    print(f"\n{'Task':<35} {'N':>4} {'V7':>6} {'V9':>6} {'Δ':>6}  {'VL#':>4} {'Evo%':>5} {'Cod%':>5} {'Add%':>5} {'Vfy%':>5} {'FF#':>4} {'t/s':>5}")
    print(f"{'-'*35} {'-'*4} {'-'*6} {'-'*6} {'-'*6}  {'-'*4} {'-'*5} {'-'*5} {'-'*5} {'-'*5} {'-'*4} {'-'*5}")

    all_v7, all_v9 = [], []
    for qt in task_types:
        qr = [r for r in all_results if r['question_type'] == qt]
        v7 = np.mean([r['v7_vl_score'] for r in qr])
        v9 = np.mean([r['score'] for r in qr])
        d = v9 - v7
        ...
        ff = np.mean([r.get('filter_frames_count', 0) for r in qr])
        t_avg = np.mean([r.get('elapsed_s', 0) for r in qr])
        mk = "+" if d > 0.01 else ("-" if d < -0.01 else "=")
        print(f"  {qt:<35} {len(qr):>4} {v7:>5.3f} {v9:>5.3f} {d:>+5.3f}{mk} {vl:>4.1f} {evo:>4.0f}% {cod:>4.0f}% {add:>4.0f}% {vfy:>4.0f}% {ff:>4.1f} {t_avg:>4.0f}s")
        all_v7.extend([r['v7_vl_score'] for r in qr])
        all_v9.extend([r['score'] for r in qr])

    print(f"{'-'*35} {'-'*4} {'-'*6} {'-'*6} {'-'*6}")
    ov7, ov9 = np.mean(all_v7), np.mean(all_v9)
    print(f"  {'Overall':<35} {len(all_results):>4} {ov7:>5.3f} {ov9:>5.3f} {ov9-ov7:>+5.3f}")
    ...
    n_ff = sum(1 for r in all_results if r.get('filter_frames_count', 0) > 0)
    total_ff = sum(r.get('filter_frames_count', 0) for r in all_results)
    print(f"  V9 features: Auto-ADD={n_add} ({100*n_add/len(all_results):.1f}%), "
          f"Verify={n_vfy} ({100*n_vfy/len(all_results):.1f}%), "
          f"FilterFrames={n_ff} ({100*n_ff/len(all_results):.1f}%), total_ff={total_ff}")
    ...
    print(f"\nResults: {output_dir}")
    print(f"\n{'='*60}")
    print(f"  V9 Overall = {ov9:.4f}  vs  V7 VL = {ov7:.4f}  (Δ = {ov9-ov7:+.4f})")
    print(f"{'='*60}")

    summary = {
        'timestamp': timestamp,
        'version': 'v9_128_temporal_filtering',
        'architecture': 'Manager(VL CodeAgent) → Gather → Auto-ADD → Critic/Evolve(FILTER_FRAMES) → Self-Verify → Decide',
        'design': 'Grid128 + Focused Grid Display + Temporal Filter + Weakened Numerical Confidence',
        'improvements': 'V5→V9: Grid128, Focused Display, FILTER_FRAMES, Low confidence for numerical tasks',
        'n_samples': len(all_results),
        'overall': {'v7_vl': float(ov7), 'v9': float(ov9), 'delta': float(ov9 - ov7)},
        'avg_vl_calls': float(avg_vl),
        'avg_time_s': float(avg_t),
        'v9_features': {
            'auto_add_count': n_add,
            'verify_count': n_vfy,
            'filter_frames_count': n_ff,
            'total_frames_filtered': int(total_ff),
        },
        'tool_usage': dict(tc),
        'by_task': {qt: {
            ...
            'v9': float(np.mean([r['score'] for r in all_results if r['question_type'] == qt])),
