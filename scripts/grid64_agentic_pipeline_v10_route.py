#!/usr/bin/env python3
"""
128³ Grid Mind Map — Agentic Pipeline V10 (MoT Closed-Loop)

核心改进 (V9→V10): Mind-of-Thought (MoT) 闭环Agent架构
  1. 真正的闭环反馈: while D > threshold → BeliefUpdate → Recompute → Recheck
  2. 统一信念更新动作空间: ADD/FILTER_FRAMES/DELETE/SCALE_ADJUST 统一为 A_int
  3. 认知失调度 D: 量化CODER结果与VL直觉的冲突程度
  4. 收敛目标: D='verified' 时停止，否则优雅降级
...
# ============================================================================
# Scheme 1: 优化后的 grid_answer_route - 添加路径可行性验证
# ============================================================================

def grid_answer_route_v2(grid: Grid64, question: str, options: List[str]) -> Tuple[str, str]:
    """v10_route: 优化路线规划 — 添加碰撞检测和路径连贯性验证
    
    改进:
    1. 添加障碍物检测 - 检查路径是否穿过其他物体
    2. 路径连贯性检查 - 确保waypoint之间的路径是连续的
    3. 转向合理性验证 - 检查转向角度是否合理
    4. 最终位置验证 - 确保能到达目标点
    """
    if not options:
        return "A", "no options"
    ...
    for eid, e in grid.entities.items():
        if eid not in [start_name.replace(' ', '_'), facing_name.replace(' ', '_')]:
            obstacle_positions.append((eid, np.array(e.position_3d, dtype=float)))
    
    n_fill_ins = sum(1 for wtype, _ in waypoints if wtype == 'fill_in')
    
    # 辅助函数: 检查线段是否接近障碍物
    def check_path_obstacles(p1, p2, exclude_names=None, threshold=0.5):
        """检查从p1到p2的路径是否太靠近其他物体"""
        if exclude_names is None:
            exclude_names = set()
        
        p1_2d = np.array([p1[0], p1[2]])
        p2_2d = np.array([p2[0], p2[2]])
        path_vec = p2_2d - p1_2d
        ...
        for eid, obs_pos in obstacle_positions:
            if any(name in eid for name in exclude_names):
                continue
            obs_2d = np.array([obs_pos[0], obs_pos[2]])
            # 计算点到线段的距离
            t = np.dot(obs_2d - p1_2d, path_unit)
            t = max(0, min(path_len, t))
            closest_point = p1_2d + t * path_unit
            dist = np.linalg.norm(obs_2d - closest_point)
            
            if dist < threshold:
                near_obstacles.append((eid, dist))
            min_clearance = min(min_clearance, dist)
        
        return min_clearance, near_obstacles
    ...
    for opt in options:
        ...
        for wi, (wtype, wname) in enumerate(waypoints):
            if wtype == 'fill_in':
                ...
                # 记录转向前的朝向
                pre_turn_facing = current_facing.copy()
                ...
                if next_wp_name and next_wp_name in wp_positions:
                    ...
                    if np.linalg.norm(to_next) > 1e-8:
                        ...
                        # 计算转向角度变化
                        turn_angle = np.arccos(np.clip(np.dot(pre_turn_facing, current_facing), -1, 1))
                ...
                if wname in wp_positions:
                    ...
                    # 检查路径是否可行 (障碍物检测)
                    exclude = {start_name.replace(' ', '_'), wname.replace(' ', '_')}
                    clearance, obstacles = check_path_obstacles(current_pos, new_pos, exclude)
                    
                    if clearance < 0.3:  # 路径太靠近障碍物
                        penalty += 2.0
                        path_valid = False
                    elif clearance < 0.6:
                        penalty += 0.5
                    ...
                    visited_waypoints.append(wname)
        
        # 最终验证: 是否到达最后一个waypoint
        final_waypoint = None
        for wtype, wname in reversed(waypoints):
            if wtype == 'go_forward' and wname:
                final_waypoint = wname
                break
        
        if final_waypoint and final_waypoint in wp_positions:
            final_dist = np.linalg.norm(current_pos - wp_positions[final_waypoint])
            if final_dist > 0.5:  # 没有到达目标
                penalty += 2.0
                path_valid = False
            else:
                score += 1.0  # 到达目标奖励
        ...
        # 计算最终得分
        final_score = score - penalty + (1.0 if path_valid else 0)
        
        score_details.append(f"{letter}: score={final_score:.2f}(raw={score:.2f},pen={penalty:.2f},valid={path_valid}) turns={opt_content}")
        if final_score > best_score:
            best_score = final_score
            best_opt = letter
    
    reasoning = f"route_v2: start={start_name} facing={facing_name}, {n_fill_ins} fill-ins, best_score={best_score:.2f}, " + "; ".join(score_details)
    return best_opt, reasoning


# 使用优化后的路由函数
grid_answer_route = grid_answer_route_v2
...
def manager_code_agent_loop(ctx: ToolExecutionContext, max_steps: int = 4) -> Tuple[str, str]:
    """
    V10: 简化的MoT闭环架构 - 基于V5逻辑，添加收敛判断
    
    核心改进:
      - 保留V5的: Gather → Auto-ADD → Critic/Evolve → Self-Verify → Decide
      - 添加: 如果Verify不通过，可以循环再试一次(最多2轮)
      - 移除: D_level人为分层，统一VL验证
    
    Architecture:
      Step 1: Gather — Manager选择actions
      Step 2: CODER计算
      Step 3: Auto-ADD — 如果not found
      Step 4: Critic/Evolve — 如果发现问题
      Step 5: Self-Verify — VL验证CODER结果
      Step 6: 如果Verify通过 → Decide
              如果Verify不通过且还有迭代次数 → 回到Step 2
      Step 7: Decide — VL最终决策
    """
    reasoning_parts = []
    MAX_ITER = 2  # 最多2轮，避免过度循环
...
class AgenticPipelineV10:
    """V10: MoT (Mind-of-Thought) Closed-Loop Agentic System"""
...
def main():
    ...
    pipeline = AgenticPipelineV10(device=args.device, vl_model_path=vl_model, max_steps=args.max_steps)
    ...
    if args.gpu_id is not None:
        output_dir = PROJECT_ROOT / "outputs" / "agentic_pipeline_v10_full" / f"gpu{args.gpu_id}"
    else:
        output_dir = PROJECT_ROOT / "outputs" / f"agentic_pipeline_v10_{timestamp}"
...
def _print_summary(all_results, output_dir, timestamp):
    print("\n" + "=" * 140)
    print("Agentic Pipeline V10 — MoT (Mind-of-Thought) Closed-Loop")
    print(f"Architecture: MoT Loop ─ Gather → ACT(CODER) → CRITIQUE(D) → EVOLVE(BeliefUpdate) → [converge?] → Decide")
    print(f"V10 Features: True Closed-Loop + Cognitive Dissonance D + Unified Belief Update + Graceful Degradation")
    print(f"Baselines: V7 VL=63.61%, V5=64.09%, V8=59.80%, V9=56.32%  |  Samples: {len(all_results)}")
    print("=" * 140)

    task_types = sorted(set(r['question_type'] for r in all_results))

    print(f"\n{'Task':<35} {'N':>4} {'V7':>6} {'V10':>6} {'Δ':>6}  {'VL#':>4} {'Evo%':>5} {'Cod%':>5} {'Add%':>5} {'Vfy%':>5} {'FF#':>4} {'t/s':>5}")
    ...
    for qt in task_types:
        ...
        print(f"  {qt:<35} {len(qr):>4} {v7:>5.3f} {v10:>5.3f} {d:>+5.3f}{mk} {vl:>4.1f} {evo:>4.0f}% {cod:>4.0f}% {add:>4.0f}% {vfy:>4.0f}% {ff:>4.1f} {t_avg:>4.0f}s")
    ...
    print(f"  V10 features: Auto-ADD={n_add} ({100*n_add/len(all_results):.1f}%), "
    ...
    summary = {
        'timestamp': timestamp,
        'version': 'v10_mot_closed_loop',
        'architecture': 'MoT Loop ─ Gather → ACT(CODER) → CRITIQUE(D) → EVOLVE(BeliefUpdate) → [converge?] → Decide',
        'design': 'True Closed-Loop + Cognitive Dissonance D + Unified Belief Update + Graceful Degradation',
        'improvements': 'V9→V10: True closed-loop, Unified D metric, Convergence target, Graceful degradation',
        'n_samples': len(all_results),
        'overall': {'v7_vl': float(ov7), 'v10': float(ov10), 'delta': float(ov10 - ov7)},
        ...
        'by_task': {qt: {
            ...
            'v10': float(np.mean([r['score'] for r in all_results if r['question_type'] == qt])),
